...
const COLLECTION_ALIAS: &str = "representative_covers";
...
const REPAIR_COLLECTION_ALIAS: &str = "representative_cover_repairs";

#[derive(DbElement, Clone, Debug)]
pub(crate) struct RepresentativeCoverProfile {
    pub(crate) db_id: Option<NodeId>,
    pub(crate) id: String,
    pub(crate) identity: String,
    pub(crate) target_kind: RepresentativeCoverTargetKind,
    pub(crate) scope: RepresentativeCoverScope,
...
#[derive(DbElement, Clone, Debug)]
pub(crate) struct RepresentativeCoverRepair {
    pub(crate) db_id: Option<NodeId>,
    pub(crate) id: String,
    pub(crate) identity: String,
    pub(crate) target_kind: RepresentativeCoverTargetKind,
...
pub(crate) enum RepresentativeCoverTargetKind {
    Genre,
    Playlist,
}

impl RepresentativeCoverTargetKind {
    ...
    fn from_db_str(value: &str) -> Result<Self, DbError> {
        match value {
            ...
            _ => Err(DbError::serialization(
                ...
                format!("invalid RepresentativeCoverTargetKind value '{value}'"),
            )),
        }
    }
}

impl fmt::Display for RepresentativeCoverTargetKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(self.as_db_str())
    }
}

impl From<RepresentativeCoverTargetKind> for DbValue {
    fn from(value: RepresentativeCoverTargetKind) -> Self {
...
impl TryFrom<DbValue> for RepresentativeCoverTargetKind {
    type Error = DbError;
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, DbTypeMarker)]
pub(crate) enum RepresentativeCoverScope {
    User,
    Instance,
}

impl RepresentativeCoverScope {
    pub(crate) const fn as_db_str(self) -> &'static str {
        match self {
            Self::User => "user",
    ...
    fn from_db_str(value: &str) -> Result<Self, DbError> {
        match value {
            ...
            _ => Err(DbError::serialization(
                agdb::DbErrorType::TypeError,
                format!("invalid RepresentativeCoverScope value '{value}'"),
            )),
        }
    }
}

impl fmt::Display for RepresentativeCoverScope {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(self.as_db_str())
    }
}

impl From<RepresentativeCoverScope> for DbValue {
    fn from(value: RepresentativeCoverScope) -> Self {
        Self::from(value.as_db_str())
    }
}
...
impl TryFrom<DbValue> for RepresentativeCoverScope {
    type Error = DbError;
...
pub(crate) enum RepresentativeCoverWinnerKind {
    Personal,
    Instance,
    Random,
}

impl RepresentativeCoverWinnerKind {
    pub(crate) const fn as_db_str(self) -> &'static str {
        match self {
...
#[derive(Clone, Debug)]
pub(crate) struct RepresentativeCoverWinner {
...
#[derive(Clone, Debug)]
struct RepresentativeCoverCandidate {
    edge_id: DbId,
...
pub(crate) struct RepresentativeCoverTarget<'a> {
    pub(crate) kind: RepresentativeCoverTargetKind,
    pub(crate) db_id: DbId,
    pub(crate) public_id: &'a str,
}

pub(crate) struct RepresentativeCoverUser<'a> {
    pub(crate) db_id: DbId,
    pub(crate) public_id: &'a str,
}
...
pub(crate) fn profile_identity(
    ...
    target_kind: RepresentativeCoverTargetKind,
    user_public_id: Option<&str>,
    target_public_id: &str,
) -> String {
    match scope {
        RepresentativeCoverScope::User => format!(
            "{PROFILE_PREFIX}:{}:{}:{}:{}",
            scope.as_db_str(),
        ...
        RepresentativeCoverScope::Instance => format!(
            "{PROFILE_PREFIX}:{}:{}:{}",
            scope.as_db_str(),
            target_kind.as_db_str(),
            target_public_id
...
fn winner_identity(profile_public_id: &str, winner_kind: RepresentativeCoverWinnerKind) -> String {
    format!(
        "{WINNER_PREFIX}:{}:{}",
        profile_public_id,
...
pub(crate) fn find_profile_by_identity(
    db: &impl DbAccess,
    identity: &str,
) -> anyhow::Result<Option<(DbId, RepresentativeCoverProfile)>> {
    for element in search_identity(db, identity)? {
        if element.id.0 <= 0
            || !super::graph::is_element_type(&element, "RepresentativeCoverProfile")
        {
            continue;
        }
        return Ok(Some((
            element.id,
            RepresentativeCoverProfile::from_db_element(&element)?,
        )));
    }
    Ok(None)
}
...
fn repair_exists(db: &impl DbAccess, identity: &str) -> anyhow::Result<bool> {
    Ok(search_identity(db, identity)?.into_iter().any(|element| {
        element.id.0 > 0 && super::graph::is_element_type(&element, "RepresentativeCoverRepair")
    }))
}

fn enqueue_repair(
    db: &mut impl DbAccess,
    target: &RepresentativeCoverTarget<'_>,
    ...
    let repair = RepresentativeCoverRepair {
        db_id: None,
        id: nanoid!(),
        identity,
    ...
    let repair_db_id = db
        ...
        .first()
        .copied()
        .ok_or_else(|| anyhow::anyhow!("representative cover repair insert returned no id"))?;
    db.exec_mut(
        QueryBuilder::insert()
            .edges()
...
fn save_profile(
    db: &mut impl DbAccess,
    profile_db_id: DbId,
    profile: &RepresentativeCoverProfile,
) -> anyhow::Result<()> {
...
pub(crate) fn ensure_profile(
    ...
    scope: RepresentativeCoverScope,
    target: &RepresentativeCoverTarget<'_>,
    user: Option<&RepresentativeCoverUser<'_>>,
    now_ms: u64,
) -> anyhow::Result<(DbId, RepresentativeCoverProfile)> {
    ...
    let profile = RepresentativeCoverProfile {
        db_id: None,
        id: nanoid!(),
        identity,
        target_kind: target.kind,
    ...
    let profile_db_id = db
        ...
        .first()
        .copied()
        .ok_or_else(|| anyhow::anyhow!("representative cover profile insert returned no id"))?;
    db.exec_mut(
        QueryBuilder::insert()
            .edges()
...
fn candidate_from_edge(edge: &DbElement) -> RepresentativeCoverCandidate {
    RepresentativeCoverCandidate {
        edge_id: edge.id,
...
fn get_candidate(
    ...
    release_public_id: &str,
) -> anyhow::Result<Option<RepresentativeCoverCandidate>> {
...
fn candidate_values(
    identity: &str,
    candidate: &RepresentativeCoverCandidate,
    now_ms: u64,
) -> Vec<DbKeyValue> {
    let mut values = vec![
        DbKeyValue {
...
fn upsert_candidate(
...
) -> anyhow::Result<RepresentativeCoverCandidate> {
    let identity = candidate_identity(profile_public_id, release_public_id);
    let mut candidate = get_candidate(db, profile_public_id, release_public_id)?.unwrap_or(
        RepresentativeCoverCandidate {
            edge_id: DbId(0),
            user_listen_count: 0,
    ...
    if candidate.edge_id.0 == 0 {
        candidate.edge_id = db
            ...
            .first()
            .copied()
            .ok_or_else(|| anyhow::anyhow!("representative cover candidate edge missing id"))?;
    }
    db.exec_mut(
        QueryBuilder::insert()
...
fn winner_from_edge(edge: &DbElement) -> RepresentativeCoverWinner {
    RepresentativeCoverWinner {
...
pub(crate) fn get_winner(
    db: &impl DbAccess,
    profile: &RepresentativeCoverProfile,
    winner_kind: RepresentativeCoverWinnerKind,
) -> anyhow::Result<Option<RepresentativeCoverWinner>> {
    Ok(
...
fn replace_winner(
    ...
    let edge_id = db
        ...
        .first()
        .copied()
        .ok_or_else(|| anyhow::anyhow!("representative cover winner edge missing id"))?;
    db.exec_mut(
        QueryBuilder::insert()
...
fn set_winner_if_better(
    ...
    let replace = match (winner_kind, current.as_ref()) {
        (_, None) => true,
        (RepresentativeCoverWinnerKind::Random, Some(current)) => {
            random_score < current.random_score
        }
...
fn release_target(
    db: &impl DbAccess,
    release_db_id: DbId,
) -> anyhow::Result<Option<(String, super::releases::Release)>> {
    let Some(release) = super::releases::get_by_id(db, release_db_id)? else {
        return Ok(None);
    };
...
fn genre_target(
    db: &impl DbAccess,
    genre_db_id: DbId,
) -> anyhow::Result<Option<(String, super::genres::Genre)>> {
    let Some(genre) = super::genres::get_by_id(db, genre_db_id)? else {
        return Ok(None);
    };
...
pub(crate) fn ensure_genre_release_random_candidate(
    ...
    if super::covers::get(db, release_db_id)?.is_none() {
        return Ok(());
    }
    ...
    let target = RepresentativeCoverTarget {
        kind: RepresentativeCoverTargetKind::Genre,
        db_id: genre_db_id,
        public_id: &genre_public_id,
    };
    let (profile_db_id, profile) = ensure_profile(
        db,
        RepresentativeCoverScope::Instance,
        &target,
        None,
...
pub(crate) fn sync_release_random_candidates(
    ...
    if super::releases::get_by_id(db, release_db_id)?.is_none()
        || super::covers::get(db, release_db_id)?.is_none()
    {
        return Ok(());
    }
    let now_ms = now_ms();
    for genre in super::genres::get_for_release(db, release_db_id)? {
...
pub(crate) fn record_genre_listen(
    ...
    let user = RepresentativeCoverUser {
        db_id: user_db_id,
        public_id: &user.id,
    };
    let releases = super::releases::get_by_track(db, track_db_id)?;
    ...
    for release in releases {
        ...
        let has_cover = super::covers::get(db, release_db_id)?.is_some();
        for genre in super::genres::get_for_release(db, release_db_id)? {
...
pub(crate) fn mark_genre_profiles_dirty_for_release(
    ...
    for genre in super::genres::get_for_release(db, release_db_id)? {
        ...
        let profiles: Vec<RepresentativeCoverProfile> = db
            .exec(
                QueryBuilder::select()
                    .elements::<RepresentativeCoverProfile>()
                    .search()
                    .from(genre_db_id)
                    .where_()
                    .neighbor()
        ...
        for profile in profiles {
            ...
            if profile.target_kind != RepresentativeCoverTargetKind::Genre {
                continue;
            }
...
pub(crate) fn mark_release_track_link_changed(
    ...
    if super::releases::get_by_id(db, from_id)?.is_some()
        && super::tracks::get_by_id(db, to_id)?.is_some()
    {
...
pub(crate) fn profile_is_clean(profile: &RepresentativeCoverProfile) -> bool {
...
mod tests {
    ...
    fn genre_listen_updates_personal_and_instance_winners() -> anyhow::Result<()> {
        ...
        let user_db_id = super::super::users::create(&mut db, &test_user("covers")?)?;
        ...
        super::super::genres::link_to_release(&mut db, genre_db_id, release_a)?;
        super::super::genres::link_to_release(&mut db, genre_db_id, release_b)?;
        ...
        let user = super::super::users::get_by_id(&db, user_db_id)?.expect("user should exist");
        let genre = super::super::genres::get_by_id(&db, genre_db_id)?.expect("genre should exist");
        ...
        let winner = get_winner(&db, &profile, RepresentativeCoverWinnerKind::Personal)?
    ...
    fn random_winner_is_deterministic_for_genre_release() -> anyhow::Result<()> {
        let mut db = new_test_db()?;
        let genre_db_id = super::super::genres::resolve_by_name(&mut db, "Jazz")?;
        let release_a = insert_release(&mut db, "A")?;
        let release_b = insert_release(&mut db, "B")?;
        super::super::genres::link_to_release(&mut db, genre_db_id, release_a)?;
        super::super::genres::link_to_release(&mut db, genre_db_id, release_b)?;
        ...
        .expect("instance profile should exist");
        let winner = get_winner(&db, &profile, RepresentativeCoverWinnerKind::Random)?
            .expect("random winner should exist");
        let release_a_public_id = super::super::releases::get_by_id(&db, release_a)?
            .unwrap()
            .id;
        let release_b_public_id = super::super::releases::get_by_id(&db, release_b)?
            .unwrap()
            .id;
    ...
    fn cover_removal_dirties_profile_and_enqueues_repair() -> anyhow::Result<()> {
        let mut db = new_test_db()?;
        let genre_db_id = super::super::genres::resolve_by_name(&mut db, "Pop")?;
        let release = insert_release(&mut db, "Covered Pop")?;
        super::super::genres::link_to_release(&mut db, genre_db_id, release)?;
        ...
        let repairs: Vec<RepresentativeCoverRepair> = db
            .exec(
                QueryBuilder::select()
                    .elements::<RepresentativeCoverRepair>()
                    .search()
                    .from("representative_cover_repairs")
                    .query(),
            )?
            .try_into()?;
