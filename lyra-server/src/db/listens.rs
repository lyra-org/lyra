...
pub(crate) fn get_stats_from_user_summaries(
    db: &DbAny,
    track_db_ids: &[DbId],
    user_db_id: DbId,
) -> anyhow::Result<Vec<ListenStats>> {
    ...
    let track_public_ids_by_db_id = super::lookup::find_ids_by_db_ids(db, &unique_ids)?;
    let requested_track_public_ids = track_public_ids_by_db_id
        .values()
        .cloned()
        .collect::<HashSet<_>>();
    if requested_track_public_ids.is_empty() {
    ...
    let mut stats_by_track = list_summaries(db, Some(user_db_id), Some(&requested_track_public_ids))?
        .into_iter()
        .map(|summary| {
            (
                summary.track_db_id,
                ListenStats {
                    db_id: summary.track_db_id,
                    count: summary.count,
