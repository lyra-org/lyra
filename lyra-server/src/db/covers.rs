...
pub(crate) fn remove(db: &mut impl DbAccess, release_db_id: DbId) -> anyhow::Result<bool> {
    ...
    super::representative_covers::mark_genre_profiles_dirty_for_release(db, release_db_id)?;
...
pub(crate) fn upsert(
    ...
    if let Some(existing) = get(db, release_db_id)? {
        ...
        super::representative_covers::sync_release_random_candidates(db, release_db_id)?;
        Ok(cover)
    } else {
