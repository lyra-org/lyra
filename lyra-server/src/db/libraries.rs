...
#[allow(dead_code)]
pub(crate) fn accessible_by_release(
    db: &impl super::DbAccess,
    principal: &crate::services::auth::Principal,
    release_db_id: DbId,
) -> anyhow::Result<Vec<LibraryView>> {
    Ok(filter_accessible_libraries(
        get_by_release(db, release_db_id)?,
        principal,
    ))
}

#[allow(dead_code)]
pub(crate) fn for_system_by_release(
    db: &impl super::DbAccess,
    _ctx: &crate::services::SystemContext,
    release_db_id: DbId,
) -> anyhow::Result<Vec<LibraryFull>> {
    Ok(get_by_release(db, release_db_id)?
        .into_iter()
        .map(LibraryFull::from)
        .collect())
}
...
pub(crate) fn accessible_for_entities(
    ...
    principal: &crate::services::auth::Principal,
    entity_ids: &[DbId],
) -> anyhow::Result<std::collections::HashMap<DbId, LibraryView>> {
...
pub(crate) fn for_system_for_entities(
    ...
    _ctx: &crate::services::SystemContext,
    entity_ids: &[DbId],
) -> anyhow::Result<std::collections::HashMap<DbId, LibraryFull>> {
    Ok(get_for_entities(db, entity_ids)?
        .into_iter()
