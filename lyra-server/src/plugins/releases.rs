...
impl ReleasesModule {
    ...
    pub(crate) async fn query(lua: Lua, plugin_id: Option<Arc<str>>, opts: Table) -> Result<Table> {
        let caller = request_caller(plugin_id)?;
    ...
    pub(crate) async fn get_by_artist(
        ...
        let mut releases = crate::db::releases::get_by_artist(&db, artist_db_id).into_lua_err()?;
        retain_accessible_releases(&db, &caller.principal, &mut releases).into_lua_err()?;
        Ok(releases)
    }
