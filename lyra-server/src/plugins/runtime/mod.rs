...
async fn declare_settings(plugin_id: Option<Arc<str>>, callback: Function) -> Result<Table> {
    let plugin_id = plugin_id
        .map(|raw| PluginId::new(raw).map_err(mlua::Error::external))
        .transpose()?;
...
async fn declare_user_settings(
...
fn get(lua: &Lua, id: String) -> Result<Value> {
    let id = require_non_empty_string(id, "id")?;
    let manifests = STATE.plugin_manifests.get();
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/plugins", "Plugins", "plugins")
        .scope_id(
            "lyra.plugins",
            "Manage installed plugins (list, install, restart, uninstall).",
            Danger::High,
        )
        .type_alias::<SettingsConfig>()
        .type_alias::<SettingsCallback>()
        .interface::<PluginManifest>()
        .interface::<SettingsChoiceOption>()
        .interface::<SettingsStringProps>()
        .interface::<SettingsNumberProps>()
        .interface::<SettingsBoolProps>()
        .interface::<SettingsChoiceProps>()
        .class::<SettingsBuilder>()
        .class::<UserSettingsAccessor>()
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("declare_settings")
                .param_type::<SettingsCallback>("callback")
                .returns::<SettingsConfig>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, callback| async move { declare_settings(plugin_id, callback).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("declare_user_settings")
                .param_type::<SettingsCallback>("callback")
                .returns::<UserSettingsAccessor>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, callback| async move {
                declare_user_settings(plugin_id, callback).await
            },
        )
        .sync_function(
            ModuleFunctionDescriptor::new("id").returns::<String>(),
            id,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("manifest")
                .returns::<PluginManifest>(),
            manifest,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("list").returns::<Vec<PluginManifest>>(),
            list,
        )
        .sync_function(
