...
mod tests {
    ...
    async fn provider_id_registration_persists_unique_and_generator() -> anyhow::Result<()> {
        ...
        assert!(spec.unique);
        assert_eq!(generator, Some(ProviderIdUrlGeneratorKind::Callback));
    ...
    async fn provider_id_registration_accepts_url_template_generator() -> anyhow::Result<()> {
        ...
        assert_eq!(
            generator,
            Some(ProviderIdUrlGeneratorKind::Template(
                "https://example.test/release/{id}".to_string()
            ))
        );
