...
async fn get_or_create(_plugin_id: Option<Arc<str>>, name: String) -> Result<DataStore> {
    {
        let db = STATE.db.read().await;
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/datastore", "DataStore", "datastore")
        .scope_id(
            "lyra.datastore",
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_or_create")
                .description("Returns a named data store, creating it if needed.")
                .param_type::<String>("name")
                .returns::<DataStore>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, name| async move { get_or_create(plugin_id, name).await },
        )
        .build()
}
