...
async fn resolve_auth(_caller: RequestCaller, bearer: Option<String>) -> Result<Value> {
...
async fn logout_session(_caller: RequestCaller, token: Option<String>) -> Result<bool> {
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/auth", "Auth", "auth")
        .scope_id(
            "lyra.auth",
            "Manage authentication sessions and tokens.",
            Danger::High,
        )
        .with_interface(AuthCapabilities::interface_descriptor())
        .with_interface(Principal::interface_descriptor())
        .with_interface(AuthCredential::interface_descriptor())
        .with_interface(ResolvedAuth::interface_descriptor())
        .with_interface(LoginResult::interface_descriptor())
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("resolve_auth")
                .description(
                    "Resolves a bearer credential to the authenticated principal and credential metadata.",
                )
                .param_type::<Option<String>>("bearer")
                .returns::<Option<ResolvedAuth>>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |_, caller, bearer| async move {
                let caller = caller?;
                resolve_auth(caller, bearer).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("logout_session")
                .description("Revokes the session identified by the provided token.")
                .param_type::<Option<String>>("token")
                .returns::<bool>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |_, caller, token| async move {
                let caller = caller?;
                logout_session(caller, token).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("login")
                .description("Attempts to log in and returns a principal plus session token.")
                .param_type::<String>("username")
                .param_type::<Option<String>>("password")
                .param_type::<Option<String>>("user_agent")
                .param_type::<Option<String>>("client_name")
                .returns::<Option<LoginResult>>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |_, caller, (username, password, user_agent, client_name)| async move {
                let caller = caller?;
                login(caller, username, password, user_agent, client_name).await
            },
        )
        .sync_function(
            ModuleFunctionDescriptor::new("capabilities")
                .description("Returns the current authentication capabilities.")
                .returns::<AuthCapabilities>(),
            |_, ()| capabilities(),
        )
        .build()
}
