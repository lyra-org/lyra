...
async fn on_update(_lua: Lua, plugin_id: Option<Arc<str>>, handler: Function) -> Result<()> {
...
async fn report(lua: Lua, plugin_id: Option<Arc<str>>, request_table: Table) -> Result<()> {
...
async fn start(lua: Lua, plugin_id: Option<Arc<str>>, request_table: Table) -> Result<i64> {
    let caller = request_caller(plugin_id)?;
...
async fn clear_session(lua: Lua, plugin_id: Option<Arc<str>>, request_table: Table) -> Result<()> {
    let caller = request_caller(plugin_id)?;
...
async fn send_command(lua: Lua, plugin_id: Option<Arc<str>>, request_table: Table) -> Result<()> {
    let caller = request_caller(plugin_id)?;
    remote::send_command(lua, &caller.principal, request_table).await
}

fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/playback_sessions", "PlaybackSessions", "playback_sessions")
        .scope_id(
            "lyra.playback_sessions",
            "Read and modify active playback sessions.",
            Danger::Medium,
        )
        .type_alias::<PlaybackUpdateHandler>()
        .interface::<PlaybackUpdatePayload>()
        .interface::<PlaybackStartRequest>()
        .interface::<PlaybackReportRequest>()
        .interface::<PlaybackSessionReportRequest>()
        .interface::<PlaybackSessionClearRequest>()
        .interface::<remote::SendCommandRequest>()
        .interface::<remote::ConnectionInfo>()
        .interface::<remote::PlaybackInfo>()
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("on_update")
                .description("Registers a callback for playback updates.")
                .param_type::<PlaybackUpdateHandler>("handler")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, handler| async move { on_update(lua, plugin_id, handler).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("report")
                .description("Reports progress for an existing playback session.")
                .param_type::<PlaybackReportRequest>("request")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, request_table| async move {
                report(lua, plugin_id, request_table).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("start")
                .description("Starts a playback session and returns its id.")
                .param_type::<PlaybackStartRequest>("request")
                .returns::<i64>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, request_table| async move {
                start(lua, plugin_id, request_table).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("report_session")
                .description(
                    "Reports plugin-scoped playback session progress and returns the session id when the playback remains active.",
                )
                .param_type::<PlaybackSessionReportRequest>("request")
                .returns::<Option<i64>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, request_table| async move {
                report_session(lua, plugin_id, request_table).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("clear_session")
                .description("Clears a plugin-scoped playback session.")
                .param_type::<PlaybackSessionClearRequest>("request")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, request_table| async move {
                clear_session(lua, plugin_id, request_table).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("list_connections")
                .description("Lists active connections for the given user with their playback state.")
                .param_type::<i64>("user_id")
                .returns::<Vec<remote::ConnectionInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, user_id| async move {
                list_connections(lua, plugin_id, user_id).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("send_command")
                .description("Sends a remote control command to a connection.")
                .param_type::<remote::SendCommandRequest>("request")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, request_table| async move {
                send_command(lua, plugin_id, request_table).await
            },
        )
        .build()
}
