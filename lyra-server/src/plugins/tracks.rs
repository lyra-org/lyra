...
impl TracksModule {
    ...
    pub(crate) async fn query(lua: Lua, caller: RequestCaller, opts: Table) -> Result<Table> {
    ...
    pub(crate) async fn get_by_ids(lua: Lua, caller: RequestCaller, ids: Table) -> Result<Table> {
    ...
    pub(crate) async fn list_many(lua: Lua, caller: RequestCaller, ids: Table) -> Result<Table> {
        let ids = parse_ids(ids)?;
        let db = STATE.db.read().await;
