...
async fn info(lua: Lua, _args: ()) -> Result<Value> {
    ...
    let setup_complete = db::roles::has_non_default_admin(&db, default_username).into_lua_err()?;
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/server", "Server", "server")
        .scope_id(
            "lyra.server",
            "Inspect and control the running server.",
            Danger::High,
        )
        .with_interface(ServerInfo::interface_descriptor())
        .async_function(
            ModuleFunctionDescriptor::new("info")
                .returns::<ServerInfo>()
                .yields(),
            info,
        )
