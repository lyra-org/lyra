...
pub(crate) async fn query(lua: Lua, caller: RequestCaller, request_table: Table) -> Result<Value> {
...
pub(crate) async fn get_type(caller: RequestCaller, id: ResolveId) -> Result<Option<String>> {
    let db = STATE.db.read().await;
...
fn module_export() -> ModuleExport {
    ModuleBuilder::new("lyra/entities", "Entities", "entities", |lua| {
    ...
    .type_alias::<EntityProjectionInfo>()
    .type_alias::<EntityIncludeSelector>()
    .type_alias::<ReleaseProjectionKind>()
    .type_alias::<TrackProjectionKind>()
    .type_alias::<ArtistProjectionKind>()
    .interface::<EntityLookupHints>()
    .interface::<ProjectionEntryInfo>()
    .interface::<CreditProjectionInfo>()
    .interface::<CreditedArtistProjectionInfo>()
    .interface::<ReleaseProjectionTrack>()
    .interface::<ReleaseProjectionIncludes>()
    .interface::<TrackProjectionIncludes>()
    .interface::<ArtistProjectionIncludes>()
    .interface::<ReleaseProjectionInfo>()
    .interface::<TrackProjectionInfo>()
    .interface::<ArtistProjectionInfo>()
    .interface::<EntityQueryRequest>()
    .interface::<EntityQueryManyRequest>()
    .class::<Release>()
    .class::<ReleaseType>()
    .class::<Artist>()
    .class::<ArtistType>()
    ...
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("query")
            .description("Queries a single typed entity projection with optional related includes.")
            .param_type::<EntityQueryRequest>("request")
            .returns::<EntityProjectionInfo>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, request| async move {
            let caller = caller?;
            query(lua, caller, request).await
        },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("query_release")
            .description("Queries a release projection with optional related includes.")
            .param_type::<EntityQueryRequest>("request")
            .returns::<ReleaseProjectionInfo>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, request| async move {
            let caller = caller?;
            query_release(lua, caller, request).await
        },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("query_track")
            .description("Queries a track projection with optional related includes.")
            .param_type::<EntityQueryRequest>("request")
            .returns::<TrackProjectionInfo>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, request| async move {
            let caller = caller?;
            query_track(lua, caller, request).await
        },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("query_artist")
            .description("Queries an artist projection with optional related includes.")
            .param_type::<EntityQueryRequest>("request")
            .returns::<ArtistProjectionInfo>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, request| async move {
            let caller = caller?;
            query_artist(lua, caller, request).await
        },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("get_type")
            .description("Returns the element type string for a given id (e.g. \"Library\", \"Release\", \"Artist\", \"Track\").")
            .param_type::<ResolveId>("id")
            .returns::<Option<String>>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |_, caller, id| async move {
            let caller = caller?;
            get_type(caller, id).await
        },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("query_many")
            .description("Queries many typed entity projections keyed by the requested id strings.")
            .param_type::<EntityQueryManyRequest>("request")
            .returns::<std::collections::BTreeMap<String, EntityProjectionInfo>>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, request| async move {
            let caller = caller?;
            query_many(lua, caller, request).await
        },
    )
    .build()
}
