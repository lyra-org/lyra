...
async fn resolve(lua: Lua, plugin_id: Option<Arc<str>>, request: Value) -> Result<NodeId> {
    let access = caller_access(plugin_id)?;
...
/// Link a child genre to a parent genre. Additive: does not remove existing parents.
async fn add_parent(
    _lua: Lua,
    plugin_id: Option<Arc<str>>,
    child_id: NodeId,
...
async fn get_by_id(lua: Lua, _plugin_id: Option<Arc<str>>, genre_id: NodeId) -> Result<Value> {
...
async fn find_by_name(lua: Lua, _plugin_id: Option<Arc<str>>, name: String) -> Result<Value> {
...
async fn get_parents(lua: Lua, _plugin_id: Option<Arc<str>>, genre_id: NodeId) -> Result<Value> {
    let db = STATE.db.read().await;
...
async fn get_children(lua: Lua, _plugin_id: Option<Arc<str>>, genre_id: NodeId) -> Result<Value> {
    let db = STATE.db.read().await;
...
async fn get_for_releases_many(
    lua: Lua,
    plugin_id: Option<Arc<str>>,
    release_ids: Table,
) -> Result<Table> {
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/genres", "Genres", "genres")
        .scope_id("lyra.genres", "Read and modify genre records.", Danger::Low)
        .with_interface(GenreExternalId::interface_descriptor())
        .with_interface(GenreAliasInput::interface_descriptor())
        .with_interface(GenreAddRequest::interface_descriptor())
        .with_interface(GenreInfo::interface_descriptor())
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("add")
                .param_type::<NodeId>("release_id")
                .param_type::<GenreAddRequest>("request")
                .returns::<NodeId>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, (release_id, request)| async move {
                add(lua, plugin_id, release_id, request).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("resolve")
                .description(
                    "Resolve or create a genre without linking it to a release.\nReturns the genre's db_id. Useful for registering parent genres that exist in the hierarchy but aren't directly tagged on a release.",
                )
                .param_type::<GenreAddRequest>("request")
                .returns::<NodeId>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, request| async move { resolve(lua, plugin_id, request).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("add_parent")
                .description(
                    "Link a child genre to a parent genre. Additive: does not remove existing parents.",
                )
                .param_type::<NodeId>("child_id")
                .param_type::<NodeId>("parent_id")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, (child_id, parent_id)| async move {
                add_parent(lua, plugin_id, child_id, parent_id).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_by_id")
                .param_type::<NodeId>("genre_id")
                .returns::<Option<GenreInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, genre_id| async move { get_by_id(lua, plugin_id, genre_id).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("find_by_name")
                .description("Case-insensitive name lookup. Aliases are not consulted; nil on miss.")
                .param_type::<String>("name")
                .returns::<Option<GenreInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, name| async move { find_by_name(lua, plugin_id, name).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_parents")
                .param_type::<NodeId>("genre_id")
                .returns::<Vec<GenreInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, genre_id| async move { get_parents(lua, plugin_id, genre_id).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_children")
                .param_type::<NodeId>("genre_id")
                .returns::<Vec<GenreInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, genre_id| async move { get_children(lua, plugin_id, genre_id).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_releases")
                .param_type::<NodeId>("genre_id")
                .returns::<Vec<NodeId>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, genre_id| async move {
                get_releases(lua, plugin_id, genre_id).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_releases_many")
                .param_type::<Vec<u64>>("genre_ids")
                .returns::<std::collections::BTreeMap<u64, Vec<u64>>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, genre_ids| async move {
                get_releases_many(lua, plugin_id, genre_ids).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_for_release")
                .param_type::<NodeId>("release_id")
                .returns::<Vec<GenreInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, release_id| async move {
                get_for_release(lua, plugin_id, release_id).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_for_releases_many")
                .param_type::<Vec<u64>>("release_ids")
                .returns::<std::collections::BTreeMap<u64, Vec<GenreInfo>>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, release_ids| async move {
                get_for_releases_many(lua, plugin_id, release_ids).await
            },
        )
        .build()
}
