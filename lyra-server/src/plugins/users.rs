...
async fn list(lua: Lua, _caller: RequestCaller, _args: ()) -> Result<Value> {
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/users", "Users", "users")
        .scope_id("lyra.users", "Read and modify user accounts.", Danger::High)
        .with_interface(PublicUser::interface_descriptor())
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("list")
                .description("Lists public users.")
                .returns::<Vec<PublicUser>>()
                .yields(),
            |lua| {
                let plugin_id = harmony_core::resolve_caller(lua);
                RequestCaller::from_lua_plugin_id(lua, plugin_id)
            },
