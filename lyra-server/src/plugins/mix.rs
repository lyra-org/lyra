...
fn from_track_callback(
    ...
    Ok(luau::ScheduledFuture::new(async move {
        let tracks = mix_service::from_seed(MixSeed::Track(seed), &options)
            .await
            .map_err(crate::plugins::runtime_error)?;
...
fn from_release_callback(
    ...
    let options = parse_consumer_options(frame.vm, frame.args.read_optional_named("opts")?)?;
    Ok(luau::ScheduledFuture::new(async move {
        let tracks = mix_service::from_seed(MixSeed::Release(seed), &options)
            .await
            .map_err(crate::plugins::runtime_error)?;
        Ok(tracks_to_luau(tracks)?)
    }))
...
fn from_artist_callback(
    ...
    let options = parse_consumer_options(frame.vm, frame.args.read_optional_named("opts")?)?;
    Ok(luau::ScheduledFuture::new(async move {
        let tracks = mix_service::from_seed(MixSeed::Artist(seed), &options)
            .await
            .map_err(crate::plugins::runtime_error)?;
        Ok(tracks_to_luau(tracks)?)
    }))
...
fn from_genre_callback(
    ...
    let options = parse_consumer_options(frame.vm, frame.args.read_optional_named("opts")?)?;
    Ok(luau::ScheduledFuture::new(async move {
        let tracks = mix_service::from_seed(MixSeed::Genre(seed), &options)
            .await
            .map_err(crate::plugins::runtime_error)?;
        Ok(tracks_to_luau(tracks)?)
    }))
...
fn from_playlist_callback(
    ...
    let options = parse_consumer_options(frame.vm, frame.args.read_optional_named("opts")?)?;
    Ok(luau::ScheduledFuture::new(async move {
        let tracks = mix_service::from_seed(MixSeed::Playlist(seed), &options)
            .await
            .map_err(crate::plugins::runtime_error)?;
        Ok(tracks_to_luau(tracks)?)
    }))
