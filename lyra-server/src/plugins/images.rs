...
async fn compose(lua: Lua, options: Value) -> mlua::Result<Value> {
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/images", "Images", "images")
        .scope_id("lyra.images", "Read and manipulate images.", Danger::Low)
        .with_interface(ComposeOptions::interface_descriptor())
        .with_interface(ComposeResult::interface_descriptor())
        .async_function(
            ModuleFunctionDescriptor::new("compose")
                .description(
                    "Compose multiple source images into a single grid image.\nReturns a cached result with path, hash, and mime_type.",
                )
                .param_type::<ComposeOptions>("options")
                .returns::<ComposeResult>()
                .yields(),
            |lua, options| async move { compose(lua, options).await },
        )
        .build()
}
