...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/listens", "Listens", "listens")
        .scope_id(
            "lyra.listens",
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_count")
                .description("Returns the listen count for a track.")
                .param_type::<i64>("track_id")
                .param_type::<Option<i64>>("user_id")
                .param_type::<Option<bool>>("merge_unique_external_ids")
                .returns::<u64>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |_, caller, (track_id, user_id, merge_unique_external_ids)| async move {
                let caller = caller?;
                get_count(caller, track_id, user_id, merge_unique_external_ids).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_counts")
                .description("Returns listen counts for many tracks.")
                .param_type::<Vec<u64>>("track_ids")
                .param_type::<Option<i64>>("user_id")
                .param_type::<Option<bool>>("merge_unique_external_ids")
                .returns::<std::collections::BTreeMap<u64, u64>>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |lua, caller, (track_ids, user_id, merge_unique_external_ids)| async move {
                let caller = caller?;
                get_counts(lua, caller, track_ids, user_id, merge_unique_external_ids).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_stats")
                .description("Returns both counts and last-played timestamps in a single scan.")
                .param_type::<Vec<u64>>("track_ids")
                .param_type::<Option<i64>>("user_id")
                .param_type::<Option<bool>>("merge_unique_external_ids")
                .returns::<std::collections::BTreeMap<
                    String,
            ...
            |lua, caller, (track_ids, user_id, merge_unique_external_ids)| async move {
                let caller = caller?;
                get_stats(lua, caller, track_ids, user_id, merge_unique_external_ids).await
            },
        )
        .build()
}
