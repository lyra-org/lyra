...
pub(crate) fn setup_docs(project_root: &Path) -> Result<()> {
    ...
    println!("Pass this flag to `luau-lsp analyze` so plugins can see Lyra globals:");
...
fn write_globals_definition(path: &Path) -> Result<()> {
    ...
    let mut contents = harmony_globals::render_plugin_log_globals_luau_definition()
        .context("render plugin log globals definition")?;
    contents.push('\n');
    contents.push('\n');
    contents.push_str(
        &harmony_task::render_global_luau_definition().context("render task global definition")?,
    );
...
fn apply_setup_globals(luaurc: &mut LuaurcConfig) {
    ...
    luaurc.merge_globals(
        harmony_globals::plugin_log_luaurc_global_names()
            .iter()
            .copied()
            .chain(harmony_task::luaurc_global_names().iter().copied()),
    );
}
