...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/labels", "Labels", "labels")
        .scope_id("lyra.labels", "Read and modify record labels.", Danger::Low)
        .with_interface(LabelExternalId::interface_descriptor())
        .with_interface(LabelAddRequest::interface_descriptor())
        .with_interface(LabelResolveRequest::interface_descriptor())
        .with_interface(LabelInfo::interface_descriptor())
        .with_interface(LabelForReleaseInfo::interface_descriptor())
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("add")
                .description(
                    "Resolve-or-create a label and link it to a release with an optional catalog number. Resolution + linking run atomically.",
                )
                .param_type::<NodeId>("release_id")
                .param_type::<LabelAddRequest>("request")
                .returns::<NodeId>()
                .yields(),
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("resolve")
                .description(
                    "Resolve or create a label without linking. Useful when a plugin needs the db_id before an unrelated operation.",
                )
                .param_type::<LabelResolveRequest>("request")
                .returns::<NodeId>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("sync_for_release")
                .description("Replace the labels on a release with the given set (authoritative sync). Mirrors what the merged-metadata pipeline does.")
                .param_type::<NodeId>("release_id")
                .param_type::<Vec<LabelAddRequest>>("requests")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, (release_id, requests)| async move {
                sync_for_release(lua, plugin_id, release_id, requests).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_by_id")
                .param_type::<NodeId>("label_id")
                .returns::<Option<LabelInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, label_id| async move { get_by_id(lua, plugin_id, label_id).await },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_for_release")
                .param_type::<NodeId>("release_id")
                .returns::<Vec<LabelForReleaseInfo>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_for_releases_many")
                ...
                .returns::<std::collections::BTreeMap<u64, Vec<LabelForReleaseInfo>>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_releases")
                ...
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, label_id| async move {
                get_releases(lua, plugin_id, label_id).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_releases_many")
                .param_type::<Vec<u64>>("label_ids")
                .returns::<std::collections::BTreeMap<u64, Vec<u64>>>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |lua, plugin_id, label_ids| async move {
                get_releases_many(lua, plugin_id, label_ids).await
            },
        )
        .build()
}
