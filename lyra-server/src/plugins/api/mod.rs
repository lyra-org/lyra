...
fn response_json_fn(lua: &Lua, args: (u16, Value, Option<Table>)) -> mlua::Result<Table> {
    build_json_response(lua, args)
}

fn response_empty_fn(lua: &Lua, args: (Option<u16>, Option<Table>)) -> mlua::Result<Table> {
    build_empty_response(lua, args)
}

fn response_text_fn(lua: &Lua, args: (u16, String, Option<Table>)) -> mlua::Result<Table> {
    build_text_response(lua, args)
}

fn response_bytes_fn(lua: &Lua, args: (u16, LuaBinaryInput, Option<Table>)) -> mlua::Result<Table> {
    build_bytes_response(lua, args)
}

fn response_redirect_fn(lua: &Lua, args: (u16, String, Option<Table>)) -> mlua::Result<Table> {
...
fn response_stream_track_fn(lua: &Lua, args: (i64, Option<Table>)) -> mlua::Result<Table> {
    build_stream_track_response(lua, args)
}

/// Builds a download-track response.
fn response_download_track_fn(lua: &Lua, args: (i64, Option<Table>)) -> mlua::Result<Table> {
...
fn response_hls_playlist_fn(lua: &Lua, args: (i64, Option<Table>)) -> mlua::Result<Table> {
    build_hls_playlist_response(lua, args)
}

fn query_bool_fn(lua: &Lua, args: (Table, String, Option<bool>)) -> mlua::Result<Option<bool>> {
...
fn query_csv_fn(lua: &Lua, args: (Table, String)) -> mlua::Result<Table> {
    query_csv_impl(lua, args)
}
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/api", "Api", "api")
        .scope_id(
            "lyra.api",
            "Register HTTP routes on the Lyra server.",
            Danger::High,
        )
        .type_alias::<JsonValue>()
        .type_alias::<ApiMethod>()
        .type_alias::<ApiRouteAuthMode>()
        .type_alias::<ApiHeaders>()
        .type_alias::<ApiQueryParams>()
        .type_alias::<ApiPathParams>()
        .type_alias::<ApiResponse>()
        .type_alias::<ApiHandler>()
        .type_alias::<ApiWebSocketHandler>()
        ...
        .interface::<HlsServeOptions>()
        .interface::<TrackServeOptions>()
        .interface::<ApiJsonResponse>()
        .interface::<ApiEmptyResponse>()
        .interface::<ApiTextResponse>()
        .interface::<ApiBytesResponse>()
        .interface::<ApiRedirectResponse>()
        .interface::<ApiFileResponse>()
        .interface::<ApiStreamTrackResponse>()
        .interface::<ApiDownloadTrackResponse>()
        .interface::<ApiHlsPlaylistResponse>()
        .class::<ApiWebSocketReader>()
        .class::<ApiWebSocketSender>()
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("route")
                .description("Register a route handler at `path` for `method`.")
                .param_type::<ApiMethod>("method")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (method, path, handler, auth_mode, case_insensitive)| async move {
                route(
                    plugin_id,
                    method,
                    path,
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get")
                ...
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                get(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("post")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
            ...
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                post(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("put")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
            ...
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                put(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("patch")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
            ...
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                patch(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("delete")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
            ...
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                delete_(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("head")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
            ...
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                head(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("options")
                .param_type::<String>("path")
                .param_type::<ApiHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .param_type::<Option<bool>>("case_insensitive")
            ...
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode, case_insensitive)| async move {
                options(plugin_id, path, handler, auth_mode, case_insensitive).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("websocket")
                .description("Register a WebSocket route.")
                .param_type::<String>("path")
                .param_type::<ApiWebSocketHandler>("handler")
                .param_type::<Option<ApiRouteAuthMode>>("auth_mode")
                .returns::<()>()
                .yields(),
            |lua| harmony_core::resolve_caller(lua),
            |_, plugin_id, (path, handler, auth_mode)| async move {
                websocket(plugin_id, path, handler, auth_mode).await
            },
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.json")
                .param_type::<u16>("status")
                .param_type::<JsonValue>("body")
                .param_type::<Option<ApiHeaders>>("headers")
                .returns::<ApiJsonResponse>(),
            response_json_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.empty")
                .param_type::<Option<u16>>("status")
                .param_type::<Option<ApiHeaders>>("headers")
                .returns::<ApiEmptyResponse>(),
            response_empty_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.text")
                .param_type::<u16>("status")
                .param_type::<String>("body")
                .param_type::<Option<ApiHeaders>>("headers")
                .returns::<ApiTextResponse>(),
            response_text_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.bytes")
                .param_type::<u16>("status")
                .param_type::<LuaBinaryInput>("body")
                .param_type::<Option<ApiHeaders>>("headers")
                .returns::<ApiBytesResponse>(),
            response_bytes_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.redirect")
                .param_type::<u16>("status")
                .param_type::<String>("location")
                .param_type::<Option<ApiHeaders>>("headers")
                .returns::<ApiRedirectResponse>(),
            response_redirect_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.file")
                .description("Builds a file response with optional image transforms.")
                .param_type::<Option<u16>>("status")
                .param_type::<String>("path")
                .param_type::<Option<ApiHeaders>>("headers")
                .param_type::<Option<ImageTransformOptions>>("transform")
                .returns::<ApiFileResponse>(),
            response_file_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.stream_track")
                .description("Builds a streamed-track response.")
                .param_type::<i64>("track_id")
                .param_type::<Option<TrackServeOptions>>("options")
                .returns::<ApiStreamTrackResponse>(),
            response_stream_track_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.download_track")
                .description("Builds a download-track response.")
                .param_type::<i64>("track_id")
                .param_type::<Option<TrackServeOptions>>("options")
                .returns::<ApiDownloadTrackResponse>(),
            response_download_track_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("response.hls_playlist")
                .description("Builds an HLS playlist response for a track.")
                .param_type::<i64>("track_id")
                .param_type::<Option<HlsServeOptions>>("options")
                .returns::<ApiHlsPlaylistResponse>(),
            response_hls_playlist_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("query.bool")
                .param_type::<ApiQueryParams>("query")
                .param_type::<String>("key")
                .param_type::<Option<bool>>("default")
                .returns::<Option<bool>>(),
            query_bool_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("query.int")
                .param_type::<ApiQueryParams>("query")
                .param_type::<String>("key")
                .param_type::<Option<i64>>("default")
                .param_type::<Option<i64>>("min")
                .param_type::<Option<i64>>("max")
                .returns::<Option<i64>>(),
            query_int_fn,
        )
        .sync_function(
            ModuleFunctionDescriptor::new("query.csv")
                .description("Splits a comma-separated query parameter into an array.")
                .param_type::<ApiQueryParams>("query")
                .param_type::<String>("key")
                .returns::<Vec<String>>(),
            query_csv_fn,
