...
async fn add(caller: RequestCaller, user_id: NodeId, target_id: NodeId) -> Result<bool> {
    ...
    let Some(target_public_id) = db::lookup::find_id_by_db_id(&*db, target_db_id).into_lua_err()?
    else {
        return Ok(false);
    };
    let outcome =
...
async fn remove(caller: RequestCaller, user_id: NodeId, target_id: NodeId) -> Result<bool> {
    if DbId::from(user_id) != caller.principal.user_db_id {
        return Ok(false);
    }
    let target_db_id: DbId = target_id.into();
...
async fn has(caller: RequestCaller, user_id: NodeId, target_id: NodeId) -> Result<bool> {
    if DbId::from(user_id) != caller.principal.user_db_id {
        return Ok(false);
    }
    let target_db_id: DbId = target_id.into();
    ...
    favorite_service::has_for_principal(&db, &caller.principal, &target_public_id).into_lua_err()
}
...
async fn list_ids(caller: RequestCaller, user_id: NodeId, entity: String) -> Result<Vec<NodeId>> {
    if DbId::from(user_id) != caller.principal.user_db_id {
        return Ok(Vec::new());
    }
    ...
    let db = STATE.db.read().await;
    let ids = favorite_service::list_ids(&db, caller.principal.user_db_id, kind).into_lua_err()?;
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/favorites", "Favorites", "favorites")
        .scope_id(
            "lyra.favorites",
        ...
        .async_function_with_prelude(
            ...
            |_, caller, (user_id, target_id)| async move {
                let caller = caller?;
                add(caller, user_id, target_id).await
            },
        )
        .async_function_with_prelude(
            ...
            |_, caller, (user_id, target_id)| async move {
                let caller = caller?;
                remove(caller, user_id, target_id).await
            },
        )
        .async_function_with_prelude(
            ...
            |_, caller, (user_id, target_id)| async move {
                let caller = caller?;
                has(caller, user_id, target_id).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("has_many")
                ...
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |lua, caller, (user_id, target_ids)| async move {
                let caller = caller?;
                has_many(lua, caller, user_id, target_ids).await
            },
        )
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("list_ids")
                .description("Errs above the server cap.")
                .param_type::<NodeId>("user_id")
                .param_type::<String>("entity")
                .returns::<Vec<NodeId>>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |_, caller, (user_id, entity)| async move {
                let caller = caller?;
                list_ids(caller, user_id, entity).await
            },
        )
        .build()
}
