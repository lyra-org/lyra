...
async fn get(lua: Lua, caller: RequestCaller, id: ResolveId) -> Result<Value> {
...
async fn get_many(lua: Lua, caller: RequestCaller, ids: Table) -> Result<Table> {
...
fn module_export() -> ModuleExport {
    ModuleBuilder::empty("lyra/covers", "Covers", "covers")
        .scope_id("lyra.covers", "Read and modify cover art.", Danger::Low)
        .with_interface(CoverInfo::interface_descriptor())
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get")
                .description("Returns a resolved cover for an entity.")
                .param_type::<ResolveId>("id")
                .returns::<Option<CoverInfo>>()
                .yields(),
        ...
        .async_function_with_prelude(
            ModuleFunctionDescriptor::new("get_many")
                .description("Returns resolved covers for many entities.")
                .param_type::<Vec<u64>>("ids")
                .returns::<std::collections::BTreeMap<u64, Option<CoverInfo>>>()
                .yields(),
            crate::plugins::caller::request_caller_from_stack,
            |lua, caller, ids| async move {
                let caller = caller?;
                get_many(lua, caller, ids).await
            },
        )
        .build()
}
