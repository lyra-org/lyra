...
fn module_export() -> ModuleExport {
    ...
    .with_interface(PlaybackSourceInfo::interface_descriptor())
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("get")
            .description("Returns resolved playback sources for tracks matching the given scope.")
            .param_type::<Option<ResolveId>>("id")
            .param_type::<Option<bool>>("include_entry")
            .returns::<Vec<PlaybackSourceInfo>>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, (id, include_entry)| async move {
            let caller = caller?;
            get(lua, caller, id, include_entry).await
        },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("get_many")
            .description("Returns resolved playback sources for many tracks.")
            .param_type::<Vec<u64>>("track_ids")
            .param_type::<Option<bool>>("include_entry")
            .returns::<std::collections::BTreeMap<u64, Option<PlaybackSourceInfo>>>()
            .yields(),
        crate::plugins::caller::request_caller_from_stack,
        |lua, caller, (track_ids, include_entry)| async move {
            let caller = caller?;
            get_many(lua, caller, track_ids, include_entry).await
        },
    )
    .build()
}
