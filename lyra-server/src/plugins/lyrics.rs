...
impl LyricsModule {
    ...
    pub(crate) async fn delete_for_track(plugin_id: Option<Arc<str>>, track_id: NodeId) -> Result<()> {
        let caller = request_caller(plugin_id)?;
        let mut db = STATE.db.write().await;
        let track_db_id = DbId::from(track_id);
        if !crate::routes::entity_accessible_to_principal(&*db, &caller.principal, track_db_id)
    ...
    pub(crate) async fn has(plugin_id: Option<Arc<str>>, track_id: NodeId) -> Result<bool> {
        let caller = request_caller(plugin_id)?;
        let track_db_id = DbId::from(track_id);
        if track_db_id.0 <= 0 {
            return Ok(false);
