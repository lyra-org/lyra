...
pub(crate) use self::workflow::{
    ...
    list_playbacks,
    pause_playing_scope_on_disconnect,
...
mod tests {
    ...
    #[tokio::test]
    async fn disconnect_pauses_playing_native_scope_at_effective_position() -> anyhow::Result<()> {
        let (mut db, _guard) = new_scoped_test_db().await?;
        let user_db_id = insert_user(&mut db, "alice")?;
        let track_db_id = insert_track(&mut db, "Track A", 200_000)?;
        ...
        let update =
            pause_playing_scope_on_disconnect(&mut db, "native", user_db_id, "device:1", 4_000)?;
        let paused = update.playback.expect("disconnect should pause playback");

        assert_eq!(update.event.as_deref(), Some("progress"));
        assert_eq!(paused.playback_session_id, started.playback_session_id);
        assert_eq!(paused.playback.position_ms, 13_000);
        assert_eq!(paused.playback.state, PlaybackState::Paused);
        assert_eq!(paused.playback.activity_ms, Some(3_000));
        assert_eq!(paused.playback.updated_at_ms, 4_000);
    ...
    #[tokio::test]
    async fn disconnect_does_not_rewrite_non_playing_native_scope() -> anyhow::Result<()> {
        let (mut db, _guard) = new_scoped_test_db().await?;
        let user_db_id = insert_user(&mut db, "alice")?;
        let track_db_id = insert_track(&mut db, "Track A", 200_000)?;

        let paused = report_playback_session(
            &mut db,
            SessionPlaybackReportRequest {
                plugin_id: "native",
                user_db_id,
        ...
        let persisted = db::playback_sessions::get_by_id(&db, paused.playback_session_id)?
            .expect("persisted playback");
        assert_eq!(persisted.position_ms, 10_000);
        assert_eq!(persisted.state, PlaybackState::Paused);
