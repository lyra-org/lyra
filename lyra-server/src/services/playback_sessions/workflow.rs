...
pub(crate) fn pause_playing_scope_on_disconnect(
    db: &mut DbAny,
    plugin_id: &str,
    user_db_id: DbId,
    session_key: &str,
    ...
    let playback = match get_playback_session(&scope) {
        Some(session) => {
            let bound = map_internal(resolve_current_playback(db, &session))?;
            match bound {
                Some(bound) if bound.playback.state == PlaybackState::Playing => {
                    let position_ms = effective_playback_position_ms(&bound.playback, now_ms);
                    Some(update_session_bound_playback(
                        db,
                        user_db_id,
                        bound,
                        PlaybackMutation {
    ...
    clear_playback_session_scope(&scope);
    finalize_optional_playback_update(db, playback, ActiveEvent::Progress, now_ms)
}
