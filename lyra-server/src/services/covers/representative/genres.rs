...
fn profile_for_genre(
    ...
    genre_public_id: &str,
) -> anyhow::Result<Option<(DbId, db_representative::RepresentativeCoverProfile)>> {
    db_representative::get_profile(
        db,
        scope,
...
fn winner_cover(
    db: &DbAny,
    profile: &db_representative::RepresentativeCoverProfile,
    kind: RepresentativeCoverWinnerKind,
    visible_release_ids: Option<&HashSet<DbId>>,
) -> anyhow::Result<Option<(db_representative::RepresentativeCoverWinner, db::Cover)>> {
    let Some(winner) = db_representative::get_winner(db, profile, kind)? else {
        return Ok(None);
    };
