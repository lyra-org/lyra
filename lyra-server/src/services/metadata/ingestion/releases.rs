...
fn desired_scan_names_match_existing(
    db: &impl DbAccess,
    existing_artist_ids: &[DbId],
    ...
    let desired: HashSet<&str> = desired_names.iter().map(String::as_str).collect();
    let mut matched = HashSet::new();
    for artist_id in existing_artist_ids {
        let Some(artist) = db::artists::get_by_id(db, *artist_id)? else {
            continue;
        };
        if desired.contains(artist.scan_name.as_str()) {
            matched.insert(artist.scan_name);
        }
    }

    Ok(matched.len() == desired.len())
}

fn resolve_artist_ids_for_owner(
    ...
    if reuse_existing {
        let existing = get_connected_artist_ids(db, owner_db_id)?;
        if !existing.is_empty() && desired_scan_names_match_existing(db, &existing, names)? {
            return Ok(existing);
        }
    }

    resolve_artist_ids(db, names, cache)
}
...
fn persist_release_inner(
    ...
    let release_artist_ids = resolve_artist_ids_for_owner(
        db,
        release_db_id,
        &release_artists,
        existing_release_id.is_some(),
        &mut artist_cache,
    )?;
    ...
    for track in release_tracks {
        ...
        let track_artist_ids = resolve_artist_ids_for_owner(
            db,
            track_db_id,
            &track_artist_names,
            is_existing_track,
