...
mod tests {
    ...
    #[test]
    fn apply_metadata_replaces_stale_combined_cv_artist_on_rescan() -> anyhow::Result<()> {
        let mut db = new_test_db()?;

        let library = test_db::insert_test_library_node(
            &mut db,
            "CV Rescan Library",
            PathBuf::from("/music/cv-rescan"),
        )?;
        let library_db_id = library.db_id.expect("test library has db_id");

        let entry_db_id = insert_entry(&mut db, "/music/cv-rescan/01.flac")?;
        connect(&mut db, library_db_id, entry_db_id)?;
        let source_key = format!("entry:{}:embedded", entry_db_id.0);
        ...
        apply_metadata(
            ...
            vec![TrackMetadata {
                entry_db_id,
                album: Some("瞬く日々へ".to_string()),
                album_artists: Some(vec!["井ノ華六花（CV.瀬戸桃子）".to_string()]),
                date: None,
                year: Some(2026),
                title: Some("瞬く日々へ".to_string()),
                artists: Some(vec!["井ノ華六花（CV.瀬戸桃子）".to_string()]),
                artist_relations: Vec::new(),
                disc: Some(1),
                disc_total: Some(1),
                track: Some(1),
        ...
        let stale_artist_id = db::lookup::find_id_by_indexed_string_field(
            &db,
            "artists",
            "scan_name",
            "scan_name",
        ...
        .ok_or_else(|| anyhow!("stale combined artist missing"))?;
        let mut stale_artist = db::artists::get_by_id(&db, stale_artist_id)?
            .ok_or_else(|| anyhow!("stale artist missing"))?;
        stale_artist.artist_name = "瀬戸桃子".to_string();
        stale_artist.artist_type = Some(db::ArtistType::Person);
        db::artists::update(&mut db, &stale_artist)?;
