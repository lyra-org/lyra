    ...
    let group_results = futures::stream::iter(scan_plan.groups.into_iter().enumerate())
        .map(|(entry_group_id, group)| {
            ...
            async move {
                ...
                if let Some(progress) = &progress {
                    progress
                        .add_counts(SyncRunCounterDelta {
                            entries_added: result.added as u64,
                            entries_updated: result.updated as u64,
                            ..Default::default()
                        })
                        .await;
                    progress
...
async fn prune_missing_entries_for_scan(
    ...
    if let Some(progress) = &progress {
        progress
            .add_counts(SyncRunCounterDelta {
                entries_deleted: prune_result.deleted as u64,
                ..Default::default()
            })
            .await;
...
async fn process_metadata_group(
    ...
    if let Some(progress) = &progress {
        progress
            .add_counts(SyncRunCounterDelta {
                releases_touched: apply_result.releases.len() as u64,
                tracks_touched: apply_result.tracks.len() as u64,
                ..Default::default()
            })
            .await;
        progress
...
async fn process_release_artifacts(
    ...
    if let Some(progress) = &progress {
        progress
            .add_counts(SyncRunCounterDelta {
                local_covers_synced: u64::from(local_cover_metadata),
                ..Default::default()
            })
            .await;
    }

    let lyrics_tracks = dispatch_release_lyrics(
        library_db_id,
        release_id,
        &plan,
        run_id.clone(),
    ...
    if let Some(progress) = &progress {
        progress
            .add_counts(SyncRunCounterDelta {
                lyrics_tracks: lyrics_tracks as u64,
                ..Default::default()
            })
            .await;
    }
    ...
    if let Some(progress) = &progress {
        progress
            .add_counts(SyncRunCounterDelta {
                provider_covers_synced: u64::from(provider_cover),
                ..Default::default()
            })
            .await;
    }
