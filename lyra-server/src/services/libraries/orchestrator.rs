...
pub(crate) struct LibrarySyncRun {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    #[schemars(description = "Run start time in epoch milliseconds.")]
    pub(crate) started_at_ms: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    #[schemars(description = "Most recent run status update time in epoch milliseconds.")]
    pub(crate) updated_at_ms: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    #[schemars(description = "Run completion time in epoch milliseconds.")]
...
pub(crate) struct LibrarySyncActiveWork {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) provider_id: Option<String>,
    #[schemars(description = "Active work stage start time in epoch milliseconds.")]
    pub(crate) stage_started_at_ms: u64,
}
    ...
    fn from(state: LibrarySyncState) -> Self {
        ...
        let active = state
            ...
            .map(|item| LibrarySyncActiveWork {
                ...
                stage_started_at_ms: unix_secs_to_ms(item.stage_started_at),
            })
            .collect();
        ...
        Self {
            run: LibrarySyncRun {
                id: state.run_id,
                status: state.status,
                started_at_ms: state.started_at.map(unix_secs_to_ms),
                updated_at_ms: state.updated_at.map(unix_secs_to_ms),
                finished_at_ms: state.finished_at.map(unix_secs_to_ms),
                error: state.error,
            },
...
fn unix_secs_to_ms(seconds: u64) -> u64 {
...
mod tests {
    ...
    fn sync_status_serialization_omits_internal_ids_and_global_stage() -> anyhow::Result<()> {
        ...
        assert_eq!(value["active"][0]["stage_started_at_ms"], 100_000);
        assert!(value["active"][0].get("stage_started_at").is_none());
        assert_eq!(value["run"]["started_at_ms"], 10_000);
        assert_eq!(value["run"]["updated_at_ms"], 20_000);
        assert!(value["run"].get("started_at").is_none());
