        ...
        Ok(()) => {
            ...
            if let Some(snapshot) = snapshot {
                tracing::info!(
                    ...
                    skipped_units = snapshot.progress.skipped_units,
...
mod tests {
    ...
    #[test]
    fn snapshot_exposes_raw_counts_without_derived_progress_fields() {
        ...
        assert_eq!(value["concurrency"]["active_units"], json!(1));
        assert_eq!(value["concurrency"]["queued_units"], json!(4));
        assert!(value["progress"].get("percent").is_none());
        assert!(value["progress"].get("processed_units").is_none());
        assert!(value["stages"][0].get("percent").is_none());
        assert!(value["stages"][0].get("processed_units").is_none());
        assert!(value["stages"][0].get("queued_units").is_none());
        assert!(value["concurrency"].get("processed_units").is_none());
        assert!(value["concurrency"].get("failed_units").is_none());
        assert!(value["concurrency"].get("active_by_stage").is_none());
    }

    #[test]
    fn idle_snapshot_uses_indeterminate_progress_without_run_id() {
