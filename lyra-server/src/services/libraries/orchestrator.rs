    ...
    fn from(state: LibrarySyncState) -> Self {
        ...
        let active = state
            ...
            .map(|item| LibrarySyncActiveWork {
                ...
                stage_started_at: unix_secs_to_rfc3339(item.stage_started_at),
            })
            .collect();
        ...
        Self {
            run: LibrarySyncRun {
                id: state.run_id,
                status: state.status,
                started_at: state.started_at.map(unix_secs_to_rfc3339),
                updated_at: state.updated_at.map(unix_secs_to_rfc3339),
                finished_at: state.finished_at.map(unix_secs_to_rfc3339),
                error: state.error,
            },
...
fn unix_secs_to_rfc3339(seconds: u64) -> String {
    crate::routes::unix_secs_to_rfc3339_u64(seconds)
}
...
mod tests {
    ...
    fn sync_status_serialization_omits_internal_ids_and_global_stage() -> anyhow::Result<()> {
        ...
        assert!(value["active"][0].get("stage_started_at_ms").is_none());
        assert_eq!(value["run"]["started_at"], "1970-01-01T00:00:10Z");
        assert_eq!(value["run"]["updated_at"], "1970-01-01T00:00:20Z");
        assert!(value["run"].get("started_at_ms").is_none());
