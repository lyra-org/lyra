...
pub(crate) fn artist_releases_by_artist(
    ...
    let mut releases_by_artist: HashMap<DbId, Vec<Release>> = unique_artist_ids
        .iter()
        .copied()
    ...
    for artist_id in unique_artist_ids {
        releases_by_artist.insert(artist_id, db::releases::get_by_artist(db, artist_id)?);
    }

    Ok(releases_by_artist)
}
...
pub(crate) fn artist_tracks_by_artist(
    ...
    let unique_artist_ids = dedupe_db_ids(artist_ids);
    let mut tracks_by_artist: HashMap<DbId, Vec<Track>> = unique_artist_ids
        .iter()
        .copied()
        .map(|artist_id| (artist_id, Vec::new()))
        .collect();
    ...
    for artist_id in unique_artist_ids {
        let mut tracks = db::tracks::get_by_artist(db, artist_id)?;
        sort_album_tracks(&mut tracks);
        tracks_by_artist.insert(artist_id, tracks);
    }
