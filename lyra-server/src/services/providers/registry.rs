...
impl ProviderRegistry {
    ...
    pub(crate) fn id_registration(
        ...
        provider_id: &str,
        id_type: &str,
    ) -> Option<(ProviderIdSpec, Option<ProviderIdUrlGeneratorKind>)> {
        ...
        let generator = provider
            ...
            .map(|generator| match generator {
                ProviderIdUrlGenerator::Callback(_) => ProviderIdUrlGeneratorKind::Callback,
                ProviderIdUrlGenerator::Template(template) => {
                    ProviderIdUrlGeneratorKind::Template(template.clone())
                }
            });
        Some((spec, generator))
    }
...
pub(crate) enum ProviderIdUrlGeneratorKind {
    Callback,
    Template(String),
}
