        ...
        )
    {
        return match mode {
            ...
            AudioVbrMode::Abr => output
                .set_raw_audio_codec_opt("abr", "1")
                .set_raw_audio_codec_opt("b", format!("{audio_bitrate_kbps}k")),
        };
    }

    output.set_raw_audio_codec_opt("b", format!("{audio_bitrate_kbps}k"))
}
...
pub(crate) fn build_hls_output(
    ...
    let mut output = Output::new(playlist_path.to_string_lossy().into_owned())
        .set_raw_format("hls")
        .set_raw_audio_codec(config.profile.ffmpeg_codec_str)
        .set_raw_format_opt("hls_time", HLS_SEGMENT_TIME_SECONDS.to_string())
        .set_raw_format_opt("hls_playlist_type", "vod")
        .set_raw_format_opt("hls_list_size", "0")
        .set_raw_format_opt("start_number", HLS_START_NUMBER.to_string())
        .set_raw_format_opt("hls_flags", "independent_segments+temp_file")
        .set_raw_format_opt("hls_segment_type", config.profile.segment_type)
        .set_raw_format_opt(
    ...
    if let Some(init_filename) = config.profile.init_filename {
        output = output.set_raw_format_opt("hls_fmp4_init_filename", init_filename);
    }
