...
use crate::{
    ...
    services::{
        ...
        covers::representative as representative_covers,
    },
};
...
async fn list_genres(
    ...
    let covers = if inc.covers {
        Some(representative_covers::genres::covers_for_genres(
            db,
            &all_genres,
    ...
    let cover = if inc.covers {
        let visible_release_ids =
            release_ids_for_accessible_libraries(db, &principal.accessible_library_ids)?
                ...
                .collect::<HashSet<_>>();
        Some(
            representative_covers::genres::cover_for_genre(
                db,
                &genre,
                &principal.user_public_id,
                Some(&visible_release_ids),
...
fn list_genres_docs(op: TransformOperation) -> TransformOperation {
    op.summary("List genres").description(
        "Returns genres attached to releases visible to the authenticated user. `library_id` scopes results to releases belonging to that public library ID. `inc=covers` includes personalized representative cover metadata. `sort_by` supports `name`, `last_played_at`, `listen_count`, `release_count`, `track_count`, `total_duration`, and `id`; `sort_order` supports `ascending` and `descending`.",
    )
}
...
fn get_genre_docs(op: TransformOperation) -> TransformOperation {
    op.summary("Get genre by ID")
        .description("Returns a single genre. Use `inc=parents,children,covers` to include hierarchy and personalized representative cover metadata.")
}
...
mod tests {
    ...
    #[tokio::test]
    async fn list_genres_does_not_leak_hidden_representative_cover() -> anyhow::Result<()> {
        let _guard = runtime_test_lock().await;
        setup_route_test().await?;
