...
async fn list_all_settings(
    ...
    let entries = manifests
        .as_ref()
        .iter()
        .map(|manifest| build_entry(&registry, &db, &manifest.id, SettingsScope::Global, None))
        .collect();

    Ok(Json(PluginSettingsListResponse { entries }))
}
...
mod tests {
    ...
    fn build_entry_returns_ready_when_schema_is_registered() -> anyhow::Result<()> {
        ...
        let mut registry = Registry::new();
        registry.register_schema(PluginId::new("demo")?, SettingsScope::Global, empty_schema())?;
    ...
    fn build_entry_returns_invalid_for_stale_stored_keys() -> anyhow::Result<()> {
        ...
        let plugin = db.transaction_mut(|t| {
            db::settings::get_or_create_plugin_settings_with(t, "demo")
        })?;
        ...
        let mut registry = Registry::new();
        registry.register_schema(PluginId::new("demo")?, SettingsScope::Global, token_schema())?;
