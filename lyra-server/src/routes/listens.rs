...
pub(super) fn get_me_listens_docs(op: TransformOperation) -> TransformOperation {
    op.summary("List my listen summaries").description(
        "Returns authenticated user-scoped per-track listen summaries as `{ items, next_cursor }`. Use `track_id` to narrow the response to one track. Without `track_id`, summaries are sorted by listen_count descending and last_played_at descending by default. Supported query parameters: `track_id`, `limit`, `cursor`, `sort_by`, `sort_order`, `merge_unique_external_ids`. `limit` defaults to 100 and is capped at 500. Drive pagination from `next_cursor`; it is `null` on the last page. Supported `sort_by` values: `listen_count`, `count`, `last_played_at`, `last_played`, `track_id`, `id`.",
    )
}

fn get_listens_docs(op: TransformOperation) -> TransformOperation {
    op.summary("List listen summaries").description(
        "Requires ManageLibraries or Admin. Returns per-user per-track listen summaries for visible library content as `{ items, next_cursor }`. Use `user_id` or `track_id` to narrow the response. Without `track_id`, summaries are sorted by listen_count descending and last_played_at descending by default. Supported query parameters: `user_id`, `track_id`, `limit`, `cursor`, `sort_by`, `sort_order`, `merge_unique_external_ids`. `limit` defaults to 100 and is capped at 500. Drive pagination from `next_cursor`; it is `null` on the last page. Supported `sort_by` values: `listen_count`, `count`, `last_played_at`, `last_played`, `user_id`, `track_id`, `id`.",
    )
}
