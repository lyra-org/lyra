...
mod tests {
    ...
    #[test]
    fn lyrics_response_uses_rfc3339_updated_at() -> anyhow::Result<()> {
        let value = serde_json::to_value(LyricsResponse {
            ...
            has_word_cues: false,
            updated_at: "2023-11-14T22:13:20.123Z".to_string(),
            lines: Vec::new(),
        })?;

        assert_eq!(value["updated_at"], json!("2023-11-14T22:13:20.123Z"));
    ...
    fn entry_response_serializes_modified_at_as_rfc3339() -> anyhow::Result<()> {
        ...
        assert!(value.get("mtime_ms").is_none());
        Ok(())
    }
}
