...
pub struct LyricsResponse {
    ...
    #[schemars(description = "Epoch milliseconds; updated only when content changes.")]
...
pub struct EntryResponse {
    ...
    pub size: u64,
    #[schemars(description = "Filesystem modification time in epoch milliseconds.")]
    pub mtime_ms: u64,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tracks: Option<Vec<TrackResponse>>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl EntryResponse {
    pub fn from_entry(entry: db::Entry, include_full_path: bool) -> Self {
        Self {
            ...
            hash: entry.hash,
            size: entry.size,
            mtime_ms: super::unix_secs_to_ms_u64(entry.mtime),
            tracks: None,
            releases: None,
            artists: None,
        }
...
mod tests {
    ...
    #[test]
    fn lyrics_response_uses_explicit_epoch_milliseconds_key() -> anyhow::Result<()> {
        let value = serde_json::to_value(LyricsResponse {
            id: "lyrics".to_string(),
            provider_id: "user".to_string(),
            language: "eng".to_string(),
            origin: LyricsOriginResponse::User,
        ...
        assert_eq!(value["updated_at_ms"], json!(1_700_000_000_123_u64));
        assert!(value.get("updated_at").is_none());
        Ok(())
    }

    #[test]
    fn entry_response_serializes_mtime_as_epoch_milliseconds() -> anyhow::Result<()> {
        ...
        assert_eq!(value["mtime_ms"], json!(1_700_000_000_000_u64));
