...
pub(crate) struct ReleaseListQuery {
    ...
    #[cfg_attr(
        feature = "docgen",
        schemars(
            description = "Comma-separated or repeated values: sort_name, name, date_created, release_date, id."
        )
    )]
    #[serde(default, deserialize_with = "deserialize_inc")]
        ...
        |err| match err {
            SortSpecParseError::UnsupportedSortByValues(values) => {
                AppError::bad_request(format!(
                    "Unsupported sort_by value(s): {}. Supported values: sort_name, name, date_created, release_date, id",
                    values.join(", ")
                ))
            }
            other => AppError::bad_request(other.to_string()),
...
mod tests {
    ...
    async fn parse_sort_specs_rejects_unsupported_values() -> anyhow::Result<()> {
        ...
        let body = to_bytes(response.into_body(), usize::MAX).await?;
        let text = std::str::from_utf8(&body)?;
        assert!(text.contains("Supported values: sort_name, name, date_created, release_date, id"));
        Ok(())
    }
