...
mod tests {
    ...
    #[cfg(feature = "docgen")]
    #[test]
    fn openapi_documents_sync_events_as_sse_items() {
        let value = serde_json::to_value(build_openapi_spec()).expect("serialize OpenAPI spec");

        assert_eq!(value["openapi"].as_str(), Some("3.2.0"));

        let content =
            value["paths"]["/api/sync/runs/{run_id}/events"]["get"]["responses"]["200"]["content"]
                .as_object()
                .expect("sync events response content");
        assert!(!content.contains_key("application/json"));

        let media_type = content
            .get("text/event-stream")
            .expect("text/event-stream response");
        assert_eq!(
            media_type["itemSchema"]["properties"]["event"]["const"].as_str(),
            Some("snapshot")
        );
        assert_eq!(
            media_type["itemSchema"]["properties"]["data"]["contentMediaType"].as_str(),
            Some("application/json")
        );
        assert_eq!(
            media_type["itemSchema"]["properties"]["data"]["contentSchema"]["$ref"].as_str(),
            Some("#/components/schemas/SyncRunEvent")
        );
        assert!(value["components"]["schemas"]["SyncRunEvent"].is_object());
    }
}
