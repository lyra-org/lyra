...
const STREAM_FORWARDER_SHUTDOWN_POLL_INTERVAL: Duration = Duration::from_millis(100);
const STREAM_TRANSCODE_WAIT_POLL_INTERVAL: Duration = Duration::from_millis(100);
const STREAM_TRANSCODE_ABORT_TIMEOUT: Duration = Duration::from_secs(2);
...
pub(crate) async fn stream_track_response(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        loop {
            ...
            let chunk = match sync_rx.recv_timeout(STREAM_FORWARDER_SHUTDOWN_POLL_INTERVAL) {
                Ok(chunk) => chunk,
    ...
    let client_disconnected_for_transcode = Arc::clone(&client_disconnected);
    let shutdown_for_transcode = shutdown.clone();
    tokio::task::spawn_blocking(move || {
        let result = match context.start() {
            Ok(mut handle) => loop {
                if shutdown_for_transcode.is_cancelled()
                    || client_disconnected_for_transcode.load(Ordering::Relaxed)
                {
                    client_disconnected_for_transcode.store(true, Ordering::Relaxed);
                    break handle.abort_with_timeout(STREAM_TRANSCODE_ABORT_TIMEOUT);
                }
                if let Some(result) =
                    handle.wait_completion_timeout(STREAM_TRANSCODE_WAIT_POLL_INTERVAL)
                {
                    break result;
                }
            },
