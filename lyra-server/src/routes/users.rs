...
struct ApiKeyResponse {
    id: String,
    name: String,
    #[schemars(description = "API key creation time in epoch milliseconds.")]
    created_at_ms: i64,
    #[schemars(description = "Most recent API key use time in epoch milliseconds.")]
...
impl From<api_keys::ApiKeyInfo> for ApiKeyResponse {
    fn from(api_key: api_keys::ApiKeyInfo) -> Self {
        Self {
            id: api_key.id,
            name: api_key.name,
            created_at_ms: super::unix_secs_to_ms_i64(api_key.created_at),
            last_used_at_ms: api_key.last_used_at.map(super::unix_secs_to_ms_i64),
        }
    }
}
...
mod tests {
    ...
    async fn api_key_routes_create_list_and_delete_current_user_keys() -> anyhow::Result<()> {
        ...
        assert!(created.last_used_at_ms.is_none());
        ...
        assert_eq!(listed[0].created_at_ms, created.created_at_ms);
