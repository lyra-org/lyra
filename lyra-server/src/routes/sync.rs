...
fn get_run_events_docs(op: TransformOperation) -> TransformOperation {
    op.summary("Stream sync run events")
        ...
        .response_with::<200, String, _>(|mut response| {
            let inner = response.inner();
            if let Some(media) = inner.content.shift_remove("text/plain; charset=utf-8") {
                inner.content.insert("text/event-stream".into(), media);
            }
