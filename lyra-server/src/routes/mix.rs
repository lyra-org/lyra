...
async fn get_mix(
    ...
    let result = if let Some(ref id) = query.seed_track {
        ...
        let mix_result = mix::from_seed(mix::MixSeed::Track(db_id), &options).await?;
...
fn mix_docs(op: TransformOperation) -> TransformOperation {
    op.summary("Generate mix")
        .description("Returns a shuffled list of tracks that share genres with the seed item. Provide exactly one of seed_track, seed_release, seed_artist, seed_genre, seed_playlist, or seed_recent. Use `inc` to hydrate nested data: `releases`, `artists`, and `release_covers` (covers require `releases`).")
}
