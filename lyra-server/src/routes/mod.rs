...
pub(crate) fn resolve_accessible_library_db_id(
    ...
    let not_found = || AppError::not_found(format!("library not found: {library_id}"));
    ...
    if principal.permissions.contains(&db::Permission::Admin) {
        ...
        let library_db_id =
            db::lookup::find_node_id_by_id(db, library_id)?.ok_or_else(not_found)?;
        db::libraries::get_by_id(db, library_db_id)?.ok_or_else(not_found)?;
