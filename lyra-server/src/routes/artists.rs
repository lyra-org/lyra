...
struct ArtistListQuery {
    ...
    #[cfg_attr(
        feature = "docgen",
        schemars(
            description = "Comma-separated or repeated values: sort_name, name, date_created, id."
        )
    )]
    #[serde(default, deserialize_with = "deserialize_inc")]
...
fn is_supported_artist_sort_key(key: SortKey) -> bool {
    matches!(
        key,
...
fn parse_artist_sort_specs(
    ...
    parse_sort_specs_tokens(sort_by, direction, is_supported_artist_sort_key, true).map_err(
        |err| match err {
            SortSpecParseError::UnsupportedSortByValues(values) => {
                AppError::bad_request(format!(
                    "Unsupported sort_by value(s): {}. Supported values: sort_name, name, date_created, id",
                    values.join(", ")
                ))
            }
            other => AppError::bad_request(other.to_string()),
...
mod tests {
    ...
    fn parse_artist_sort_specs_accepts_supported_values() -> anyhow::Result<()> {
        ...
        assert!(matches!(specs[3].key, SortKey::DbId));
        assert!(
            specs
                .iter()
