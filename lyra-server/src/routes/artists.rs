...
struct ArtistListQuery {
    ...
    #[cfg_attr(
        feature = "docgen",
        schemars(description = "Comma-separated or repeated values: sort_name, name, date_created, id.")
    )]
    #[serde(default, deserialize_with = "deserialize_inc")]
...
async fn get_artists(
    ...
    Ok(Json(
        list_artist_responses(&principal, inc, query, library_id, sort_by, sort_order, page)
            .await?,
    ))
}
