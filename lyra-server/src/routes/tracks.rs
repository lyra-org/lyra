...
struct TrackQuery {
    #[cfg_attr(
        feature = "docgen",
            schemars(description = "Comma-separated or repeated values: releases, artists, release_covers.")
    )]
...
#[derive(Clone, Copy)]
struct TrackRouteIncludes {
    service: track_service::TrackIncludes,
    release_covers: bool,
}

fn parse_inc(inc: Option<Vec<String>>) -> Result<TrackRouteIncludes, AppError> {
...
fn track_detail_to_response(
    ...
    let releases = detail.releases.map(|releases| {
        releases
            .into_iter()
...
fn get_track_docs(op: TransformOperation) -> TransformOperation {
    op.summary("Get track by ID").description(
        "Returns a single track. 404 if not found. Use `inc` to include releases and/or artists. When `inc=release_covers`, nested release metadata includes a public cover image URL. When `inc=artists`, each artist carries a `credit` object with `type`, `detail`, and `source`. An artist may appear multiple times with different credits. Artists without direct track credits inherit from the release (`source: release`).",
    )
}
