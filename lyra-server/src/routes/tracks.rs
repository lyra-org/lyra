...
struct TrackListQuery {
    ...
    #[cfg_attr(
        feature = "docgen",
        schemars(
            description = "Comma-separated or repeated values: sort_name, name, date_created, duration, id. When release_id is present, disc and track are also supported."
        )
    )]
    #[serde(default, deserialize_with = "deserialize_inc")]
...
fn default_track_sort(release_scoped: bool) -> Vec<SortSpec> {
    if release_scoped {
        return vec![
            SortSpec {
                key: SortKey::DiscNumber,
                direction: SortDirection::Ascending,
            },
            SortSpec {
                key: SortKey::TrackNumber,
                direction: SortDirection::Ascending,
            },
            SortSpec {
...
fn track_sort_supported_values(release_scoped: bool) -> &'static str {
    if release_scoped {
        "sort_name, name, date_created, disc, track, duration, id"
    } else {
        "sort_name, name, date_created, duration, id"
    }
}
...
fn parse_track_sort_specs(
    ...
    parse_sort_specs_tokens(
        sort_by,
        direction,
        |key| is_supported_track_sort_key(key, release_scoped),
        true,
    )
    .map_err(|err| match err {
        SortSpecParseError::UnsupportedSortByValues(values) => AppError::bad_request(format!(
...
mod tests {
    ...
    fn parse_track_sort_specs_accepts_global_supported_values() -> anyhow::Result<()> {
        let specs = match parse_track_sort_specs(
            Some(vec![
                ...
                "date_created,duration,id".to_string(),
            ]),
        ...
        assert!(matches!(specs[2].key, SortKey::DateCreated));
        assert!(matches!(specs[3].key, SortKey::Duration));
        assert!(matches!(specs[4].key, SortKey::DbId));
        assert!(
            specs
                .iter()
    ...
    fn parse_track_sort_specs_scopes_disc_and_track_to_release_filter() -> anyhow::Result<()> {
        ...
        assert!(matches!(specs[0].key, SortKey::DiscNumber));
        assert!(matches!(specs[1].key, SortKey::TrackNumber));
        Ok(())
    }
