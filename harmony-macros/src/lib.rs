...
impl FromMeta for ModuleContext {
    fn from_string(value: &str) -> darling::Result<Self> {
        match value {
            ...
            other => Err(darling::Error::custom(format!(
                "unsupported module context '{other}'; expected request, system, caller, or plugin"
            ))),
        }
    }
}
...
pub fn module(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for item in &mut ast.items {
        ...
        let caller_context_type =
            module_caller_context_type(module_options.context, &fn_item.sig.inputs);
...
fn parse_module_options(attr: TokenStream) -> syn::Result<ModuleOptions> {
    ...
    options.context = scalar.context;
    options.plugin_scoped = scalar.plugin_scoped;
    validate_module_context(&options)?;
    Ok(options)
}

fn validate_module_context(options: &ModuleOptions) -> syn::Result<()> {
    if options.context.is_some() && !options.plugin_scoped {
        return Err(syn::Error::new(
            Span::call_site(),
            "module context is only valid with plugin_scoped",
        ));
    }
    ...
    if options
        .path
        .as_deref()
        .is_some_and(|path| path.starts_with("lyra/"))
        && options.plugin_scoped
        && options.context.is_none()
    {
        return Err(syn::Error::new(
            Span::call_site(),
            "lyra plugin modules using plugin_scoped must declare context = \"request\", \"system\", \"caller\", or \"plugin\"",
        ));
    }
...
fn module_registration_tokens(
    ...
    if let Some(caller_ty) = caller_context_type {
        let trait_path = match module_context {
            Some(ModuleContext::Request) => quote! { ::harmony_core::RequestModuleContext },
            Some(ModuleContext::System) => quote! { ::harmony_core::SystemModuleContext },
            _ => {
                return quote! {
                    compile_error!("typed module caller arguments require context = \"request\" or context = \"system\"");
                };
            }
        };
        let prelude = quote! {
            |lua| {
                ...
                <#caller_ty as #trait_path>::from_lua_plugin_id(lua, __harmony_plugin_id)
            }
        };

        let async_invocation = if has_lua_context {
            if returns_result {
                quote! { Self::#fn_name(lua, __harmony_caller, #(#param_names,)*).await }
            } else {
                quote! { Ok(Self::#fn_name(lua, __harmony_caller, #(#param_names,)*).await) }
            }
        } else if returns_result {
            quote! { Self::#fn_name(__harmony_caller, #(#param_names,)*).await }
        } else {
            quote! { Ok(Self::#fn_name(__harmony_caller, #(#param_names,)*).await) }
        };

        let sync_invocation = if has_lua_context {
            if returns_result {
                quote! { Self::#fn_name(lua, __harmony_caller, #(#param_names,)*) }
            } else {
                quote! { Ok(Self::#fn_name(lua, __harmony_caller, #(#param_names,)*)) }
            }
        } else if returns_result {
            quote! { Self::#fn_name(__harmony_caller, #(#param_names,)*) }
        } else {
            quote! { Ok(Self::#fn_name(__harmony_caller, #(#param_names,)*)) }
        };
        ...
        .filter_map(|argument| match argument {
            FnArg::Typed(pat_type) => {
                if is_lua_context_type(pat_type.ty.as_ref())
                    ...
                    || is_module_caller_context_type(pat_type.ty.as_ref())
                {
                    return None;
                }
...
fn module_caller_context_type(
    context: Option<ModuleContext>,
    inputs: &Punctuated<FnArg, Token![,]>,
) -> Option<&Type> {
    let expected = match context {
        Some(ModuleContext::Request) => "RequestCaller",
        Some(ModuleContext::System) => "SystemCaller",
        _ => return None,
    };

    inputs.iter().find_map(|argument| {
        let FnArg::Typed(pat_type) = argument else {
            return None;
        };
        if type_last_ident(pat_type.ty.as_ref()).is_some_and(|ident| ident == expected) {
...
fn is_module_caller_context_type(ty: &Type) -> bool {
    type_last_ident(ty).is_some_and(|ident| ident == "RequestCaller" || ident == "SystemCaller")
}
