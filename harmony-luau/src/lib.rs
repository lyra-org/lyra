...
impl FunctionParameter {
    pub fn named(name: &'static str, ty: LuauType) -> Self {
        Self {
            name: Some(name),
            ty,
            variadic: false,
    ...
    pub fn unnamed(ty: LuauType) -> Self {
        Self {
            name: None,
            ty,
            variadic: false,
...
impl ParameterDescriptor {
    pub fn new(name: &'static str, ty: LuauType) -> Self {
        Self {
            name,
            ty,
            description: None,
...
impl MethodDescriptor {
    pub fn new(kind: MethodKind, name: &'static str) -> Self {
        Self {
            name,
            description: None,
            params: Vec::new(),
    ...
    pub fn instance(name: &'static str) -> Self {
        Self::new(MethodKind::Instance, name)
    }

    pub fn static_method(name: &'static str) -> Self {
        Self::new(MethodKind::Static, name)
    }
    ...
    pub fn param_type<T>(self, name: &'static str) -> Self
    where
        T: LuauTypeInfo,
    {
        self.param(ParameterDescriptor::of::<T>(name))
    }

    pub fn returns<T>(mut self) -> Self
    where
        T: LuauReturn,
    {
        self.returns = T::luau_returns();
        self
    }

    pub fn return_type(mut self, ty: LuauType) -> Self {
...
impl ClassDescriptor {
    ...
    pub fn field_type<T>(self, name: &'static str) -> Self
    where
        T: LuauTypeInfo,
    {
        self.field(FieldDescriptor::of::<T>(name))
    }

    pub fn variant(self, name: &'static str) -> Self {
        let ty = LuauType::literal(self.name);
        self.field(FieldDescriptor::new(name, ty))
    }
...
impl ModuleFunctionDescriptor {
    pub fn new(path: &'static str) -> Self {
        Self {
            path: split_luau_path(path),
            description: None,
            params: Vec::new(),
            returns: Vec::new(),
            yields: false,
...
impl ModuleDefinition {
    pub fn new(
        ...
        Self {
            module: ModuleDescriptor::new(name, local_name, description),
            aliases: Vec::new(),
            interfaces: Vec::new(),
            classes: Vec::new(),
        }
    ...
    pub fn from_descriptor(module: ModuleDescriptor) -> Self {
        Self {
            module,
            aliases: Vec::new(),
            interfaces: Vec::new(),
    ...
    pub fn description(mut self, description: &'static str) -> Self {
        self.module.description = Some(description);
        self
    }
    ...
    pub fn render_definition_file(&self) -> Result<String, fmt::Error> {
...
impl From<ModuleDescriptor> for ModuleDefinition {
    fn from(module: ModuleDescriptor) -> Self {
        Self::from_descriptor(module)
    }
}
...
fn split_luau_path(path: &'static str) -> Vec<&'static str> {
    let segments = path.split('.').collect::<Vec<_>>();
    assert!(
        !segments.is_empty() && segments.iter().all(|segment| !segment.is_empty()),
        "Luau paths must contain non-empty dot-separated segments"
    );
    segments
}
...
mod tests {
    ...
    #[test]
    fn module_definition_builder_renders_support_items() {
        let interface = InterfaceDescriptor::new("RequestMeta", Some("Request context."))
            .field_type::<Option<String>>("trace_id");

        let class = ClassDescriptor::new("Player", Some("Example userdata class."))
            .field_type::<String>("name")
            .method(
                MethodDescriptor::static_method("new")
                    .description("Creates a player.")
                    .param_type::<String>("name")
                    .return_type(LuauType::literal("Player")),
            )
            .method(
                MethodDescriptor::instance("rename")
                    .description("Renames the player.")
                    .param_type::<String>("name")
                    .returns::<()>(),
            );

        let definition = ModuleDefinition::new("Demo", "demo", Some("Prototype module."))
            .type_alias::<JsonValue>()
            .with_interface(interface)
            .with_class(class)
            .function(
                ModuleFunctionDescriptor::new("make_player")
    ...
    #[test]
    fn class_builder_can_describe_enum_variants() {
        let class = ClassDescriptor::new("HttpMethod", None)
            .variants(["Get", "Post", "Put", "Delete", "Patch", "Head"]);

        assert_eq!(class.fields.len(), 6);
        assert_eq!(class.fields[0].name, "Get");
        assert_eq!(class.fields[0].ty, LuauType::literal("HttpMethod"));
        assert!(is_enum_class(&class));
    }
