...
impl Luau {
    ...
    {
        create_callback::<A, F, R>(self.state(), debug_name, callback)
    }
...
impl<'lua> LuauTable<'lua> {
    ...
    {
        let function = create_callback::<A, F, R>(self.state, debug_name, callback)?;
...
struct CallbackHolder<A, F, R> {
    debug_name: CString,
    callback: F,
    _marker: PhantomData<fn(A) -> R>,
}

fn create_callback<'lua, A, F, R>(
    state: LuauState<'lua>,
    debug_name: &str,
    callback: F,
    ...
    for<'state> A: FromLuauMulti<'state>,
{
    ...
    let storage = unsafe {
        sys::lua_newuserdatadtor(
            state.as_ptr(),
            mem::size_of::<CallbackHolder<A, F, R>>(),
            Some(callback_drop::<A, F, R>),
        )
    };
    let Some(storage) = NonNull::new(storage.cast::<CallbackHolder<A, F, R>>()) else {
    ...
    unsafe {
        sys::lua_pushcclosure(
            state.as_ptr(),
            Some(callback_trampoline::<A, F, R>),
            debug_name,
            1,
        )
    };
...
unsafe extern "C-unwind" fn callback_drop<A, F, R>(userdata: *mut c_void) {
    unsafe { ptr::drop_in_place(userdata.cast::<CallbackHolder<A, F, R>>()) };
}

unsafe extern "C-unwind" fn callback_trampoline<A, F, R>(raw_state: *mut sys::lua_State) -> c_int
where
    A: 'static,
    F: for<'state> Fn(LuauState<'state>, A) -> Result<R, LuauError> + 'static,
    R: IntoLuauMulti + 'static,
    for<'state> A: FromLuauMulti<'state>,
{
    ...
    let holder = unsafe {
        sys::lua_touserdata(raw_state, sys::lua_upvalueindex(1)).cast::<CallbackHolder<A, F, R>>()
    };
    if holder.is_null() {
        unsafe { raise_luau_error(raw_state, "missing Rust callback state") };
    }

    let args = match unsafe { A::from_stack(state, 1) } {
        Ok(args) => args,
        Err(error) => unsafe { raise_luau_error(raw_state, error) },
    };

    let result = catch_unwind(AssertUnwindSafe(|| unsafe {
        ((*holder).callback)(state, args)
    }));
    match result {
        Ok(Ok(value)) => {
            if let Err(error) = value.push_to_stack(state) {
                unsafe { raise_luau_error(raw_state, error) };
            }
            R::VALUE_COUNT
        }
