...
impl std::fmt::Display for LuauError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidArrayIndex(index) => write!(f, "invalid Luau array index: {index}"),
...
pub type CapturedContext = Box<dyn Any>;

/// Re-evaluate before stabilizing. Capture runs synchronously before an async
/// callback yields, while caller stack information is still available.
pub trait CallbackCapture: 'static {
    fn capture<'lua>(&self, state: LuauState<'lua>) -> Result<CapturedContext, LuauError>;
}

impl<F> CallbackCapture for F
where
    F: for<'lua> Fn(LuauState<'lua>) -> Result<CapturedContext, LuauError> + 'static,
{
    fn capture<'lua>(&self, state: LuauState<'lua>) -> Result<CapturedContext, LuauError> {
        self(state)
    }
...
/// Re-evaluate before stabilizing. Options keep callback construction
/// extensible without creating separate method families for each policy.
#[derive(Default)]
pub struct AsyncCallbackOptions {
    capture: Option<Box<dyn CallbackCapture>>,
}

impl AsyncCallbackOptions {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn capture<C>(mut self, capture: C) -> Self
    where
        C: CallbackCapture,
    {
...
impl Luau {
    ...
    /// Re-evaluate before stabilizing. Raw-state escape hatches are useful for
    /// low-level integration but should stay narrowly documented.
    pub fn as_ptr(&self) -> *mut sys::lua_State {
        self.state().as_ptr()
    }
    ...
    /// Re-evaluate before stabilizing. Capture is modeled as options on the
    /// callback value instead of a separate callback family.
    pub fn create_async_function_with_options<A, F, R, Fut>(
        &self,
        debug_name: &str,
    ...
    where
        A: 'static,
        F: for<'lua> Fn(LuauState<'lua>, Option<CapturedContext>, A) -> Fut + 'static,
        R: IntoLuauMulti + 'static,
        Fut: Future<Output = Result<R, LuauError>> + 'static,
        for<'lua> A: FromLuauMulti<'lua>,
    {
        create_callback(
            ...
            CallbackKind::Async {
                scheduler: Rc::clone(&self.scheduler),
                callback: Box::new(CapturingTypedAsyncCallback::<A, F, R, Fut> {
                    options,
                    callback,
                    _marker: PhantomData,
                }),
...
struct CapturingTypedAsyncCallback<A, F, R, Fut> {
    options: AsyncCallbackOptions,
    callback: F,
    _marker: PhantomData<fn(A) -> (R, Fut)>,
}

impl<A, F, R, Fut> AsyncCallback for CapturingTypedAsyncCallback<A, F, R, Fut>
where
    A: 'static,
    F: for<'lua> Fn(LuauState<'lua>, Option<CapturedContext>, A) -> Fut + 'static,
    R: IntoLuauMulti + 'static,
...
{
    fn call<'lua>(&self, state: LuauState<'lua>) -> Result<ScheduledFuture, LuauError> {
        let captured = match &self.options.capture {
            Some(capture) => Some(capture.capture(state)?),
            None => None,
        };
        let args = unsafe { A::from_stack(state, 1)? };
        let future = (self.callback)(state, captured, args);
        Ok(Box::pin(async move {
            let value = future.await?;
            Ok(Box::new(TypedAsyncResult(value)) as Box<dyn AsyncResult>)
        }))
...
mod tests {
    ...
    fn capture_test_context(_: super::LuauState<'_>) -> Result<CapturedContext, LuauError> {
        Ok(Box::new("captured".to_string()) as CapturedContext)
    }
    ...
    fn sets_rust_function_on_table() {
        ...
        let answer = luau
            .create_function("api.answer", |_, ()| Ok(42.0))
            .expect("create Rust function");
        api.set("answer", &answer).expect("set table function");
    ...
    fn sets_async_rust_function_on_table() {
        ...
        let api = luau.create_table();
        let answer = luau
            .create_async_function("api.answer", |_, ()| async { Ok(42.0) })
            .expect("create async Rust function");
        api.set("answer", &answer).expect("set table function");
        luau.set_global("api", &api).expect("set global");
        let mut thread = Box::pin(
    ...
    #[test]
    fn captures_context_before_async_callback_yields() {
        let luau = Luau::new().expect("create Luau state");
        let captured = AsyncCallbackOptions::new().capture(capture_test_context);
        let read_context = luau
            .create_async_function_with_options(
                "read_context",
                captured,
                |_, captured, suffix: String| async move {
                    let prefix = *captured
                        .expect("capture should exist")
                        .downcast::<String>()
                        .expect("capture should be a string");
                    Ok(format!("{prefix}:{suffix}"))
                },
            )
            .expect("create captured async Rust function");
        luau.set_global("read_context", &read_context)
            .expect("set global");
        let mut thread = Box::pin(
            luau.spawn::<String>("unit/captured-async-callback", "return read_context('ok')")
                .expect("spawn Luau thread"),
        );
