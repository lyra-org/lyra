...
fn schedule_task(
    ...
    if let Some(delay) = delay {
        scheduler.schedule_luau_thread_after(context, delay, frame.vm.clone(), thread.clone(), args);
    } else {
...
mod tests {
    ...
    #[test]
    fn delayed_thread_args_survive_early_spawn() -> harmony_luau::runtime::Result<()> {
        let vm = harmony_luau::Vm::new()?;
        vm.open_standard_libraries(harmony_luau::StandardLibraries {
            base: true,
            coroutine: true,
        ...
        vm.eval(
            std::sync::Arc::<[u8]>::from(
                &br#"
                    observed = {}
                    local thread = task.delay(0, function(foo, extra)
                        table.insert(observed, "foo=" .. tostring(foo))
                        table.insert(observed, "extra=" .. tostring(extra))
                        table.insert(observed, "yield=" .. tostring(coroutine.yield()))
        ...
        assert_eq!(
            vm.eval(
                std::sync::Arc::<[u8]>::from(&b"return table.concat(observed, ',')"[..]),
                harmony_luau::ChunkOrigin::default(),
            )?,
            vec![harmony_luau::Value::String(
                b"foo=b,extra=a,yield=d,done,manual=true:nil".to_vec()
            )]
        );
        assert!(
            scheduler
                .snapshots()
                .into_iter()
                .all(|snapshot| snapshot.state != harmony_core::TaskState::Failed)
        );
        Ok(())
    }

    #[test]
    fn later_delay_does_not_consume_queued_spawn() -> harmony_luau::runtime::Result<()> {
        let vm = harmony_luau::Vm::new()?;
        vm.open_standard_libraries(harmony_luau::StandardLibraries {
            base: true,
            coroutine: true,
        ...
        vm.eval(
            std::sync::Arc::<[u8]>::from(
                &br#"
                    observed = {}
                    local thread = task.spawn(function(foo, extra)
                        table.insert(observed, "foo=" .. tostring(foo))
                        table.insert(observed, "extra=" .. tostring(extra))
                        table.insert(observed, "yield=" .. tostring(coroutine.yield()))
                    end, "a")

                    task.delay(10, thread, "b")
                    task.spawn(function()
                        local ok, value = coroutine.resume(thread, "d")
                        table.insert(observed, "manual=" .. tostring(ok) .. ":" .. tostring(value))
                    end)
        ...
        assert_eq!(
            ...
            vec![harmony_luau::Value::String(
                b"foo=a,extra=nil,yield=d,manual=true:nil".to_vec()
            )]
        );
        Ok(())
    }
