...
const TASK_GLOBAL_NAME: &str = "task";
const LUAURC_GLOBAL_NAMES: &[&str] = &[TASK_GLOBAL_NAME];
...
pub fn install_global(vm: &luau::Vm, origin: &ChunkOrigin) -> luau::runtime::Result<()> {
    let table = harmony_core::install_luau_module(vm, origin, &module_spec())?;
    vm.set_global_table(TASK_GLOBAL_NAME, &table)
}

pub fn luaurc_global_names() -> &'static [&'static str] {
    LUAURC_GLOBAL_NAMES
}
...
fn render_luau_definition() -> Result<String, std::fmt::Error> {
    render_definition_file(&TaskModuleDocs::module_descriptor(), &[])
}

pub fn render_global_luau_definition() -> Result<String, std::fmt::Error> {
    let mut rendered = render_luau_definition()?;
    let return_line = format!("return {TASK_GLOBAL_NAME}\n");
    if rendered.ends_with(&return_line) {
        rendered.truncate(rendered.len() - return_line.len());
    }
    rendered.push_str(&format!("declare {TASK_GLOBAL_NAME}: typeof({TASK_GLOBAL_NAME})\n"));
    Ok(rendered)
}
...
mod tests {
    use super::{
        ...
        luaurc_global_names,
        module_spec,
        render_global_luau_definition,
    };

    #[test]
    fn renders_task_global_definition() {
        let rendered = render_global_luau_definition().expect("render task global docs");
        ...
        assert!(rendered.contains("declare task: typeof(task)"));
        assert!(!rendered.contains("return task"));
    }
    ...
    fn exposes_handwritten_module_spec() {
        ...
        assert_eq!(luaurc_global_names(), &["task"]);
    }

    #[test]
    fn luau_global_schedules_spawn_and_cancel() -> harmony_luau::runtime::Result<()> {
        ...
        install_global(&vm, &harmony_core::ChunkOrigin::default())?;
