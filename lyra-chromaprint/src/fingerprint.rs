...
impl Chroma {
    ...
    pub fn filter(&mut self, fft_frame: &[Float; FFT_FRAME_SIZE]) -> Option<&[Float; BANDS_LEN]> {
        self.filter_energies(|index| fft_frame[index])
    }

    pub(crate) fn filter_energies(
        &mut self,
        mut energy_at: impl FnMut(usize) -> Float,
    ) -> Option<&[Float; BANDS_LEN]> {
        let table = &*CHROMA_TABLE;
        {
            let buf = &mut self.buffer[self.buffer_i];
            buf.fill(0.0);
            for (i, &note) in table.notes[table.min_index..table.max_index]
                .iter()
                .enumerate()
            {
                let energy = energy_at(table.min_index + i);
