...
fn plan_fft() -> Arc<dyn Fft<f32>> {
    #[cfg(target_arch = "x86_64")]
    if let Ok(mut planner) = FftPlannerAvx::new() {
        return planner.plan_fft_forward(WINDOW_SIZE);
    }

    Arc::new(Radix4::new(WINDOW_SIZE, FftDirection::Forward))
}
...
pub fn compute_fingerprint_from_samples(samples: &[i16], duration_secs: Option<u32>) -> Vec<u32> {
    ...
    let mut output = vec![Complex::new(0.0f32, 0.0f32); WINDOW_SIZE];
    let fft = plan_fft();
    ...
    while offset + WINDOW_SIZE <= samples.len() {
        ...
        if let Some(features) = chroma.filter_energies(|i| {
            let c = output[i];
            c.re * c.re + c.im * c.im
        }) {
