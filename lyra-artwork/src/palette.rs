...
fn render_palette(selected: &[Candidate], candidates: &[Candidate], seed: &str) -> Vec<PaletteColor> {
    ...
    let mut accents = candidates
        ...
        .collect::<Vec<_>>();
    accents.sort_by(|a, b| {
        compare_f32(accent_score(b), accent_score(a)).then_with(|| a.tie_breaker.cmp(&b.tie_breaker))
    });
