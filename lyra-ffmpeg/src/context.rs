    ...
    pub fn wait_completion_timeout(&mut self, timeout: Duration) -> Option<Result<()>> {
        ...
        if let Some(thread) = self.thread.take() {
            if thread.join().is_err() {
                return Some(Err(Error::ThreadJoin));
            }
        }
        Some(
            self.result
...
mod tests {
    ...
    fn make_test_handle<F>(worker: F) -> FfmpegHandle
    where
        F: FnOnce(Arc<AtomicBool>, Arc<Mutex<Option<Result<()>>>>, Arc<(Mutex<bool>, Condvar)>)
            + Send
            + 'static,
    {
        let abort_flag = Arc::new(AtomicBool::new(false));
        ...
        let thread = {
            ...
            let done = done.clone();
            std::thread::spawn(move || worker(abort_flag, result, done))
        };
    ...
    #[test]
    fn wait_completion_timeout_returns_finished_result() {
        let mut handle = make_test_handle(|_abort_flag, result, done| {
    ...
    #[test]
    fn abort_with_timeout_detaches_unfinished_worker() {
        let handle = make_test_handle(|abort_flag, _result, _done| {
        ...
        let error = handle
            .abort_with_timeout(Duration::from_millis(5))
