...
impl mlua::UserData for HttpMethod {
    fn add_fields<F: mlua::UserDataFields<Self>>(fields: &mut F) {
        fields.add_field_function_get("Get", |_, _| Ok(HttpMethod::Get));
        fields.add_field_function_get("Post", |_, _| Ok(HttpMethod::Post));
        fields.add_field_function_get("Put", |_, _| Ok(HttpMethod::Put));
        fields.add_field_function_get("Delete", |_, _| Ok(HttpMethod::Delete));
        fields.add_field_function_get("Patch", |_, _| Ok(HttpMethod::Patch));
        fields.add_field_function_get("Head", |_, _| Ok(HttpMethod::Head));
    }
...
impl FromLua for HttpMethod {
    fn from_lua(value: Value, _lua: &Lua) -> mlua::Result<Self> {
        match value {
            Value::UserData(user_data) => match user_data.borrow::<Self>() {
                Ok(value) => Ok(*value),
                Err(_) => Err(mlua::Error::FromLuaConversionError {
                    from: "UserData",
                    to: "HttpMethod".to_string(),
            ...
            _ => Err(mlua::Error::FromLuaConversionError {
                ...
                to: "HttpMethod".to_string(),
                message: Some("expected userdata created by harmony-http".to_string()),
            }),
        }
    }
}
...
impl DescribeUserData for HttpMethod {
    fn class_descriptor() -> ClassDescriptor {
        ClassDescriptor::new("HttpMethod", None)
...
impl DescribeInterface for HttpRequestOptions {
    fn interface_descriptor() -> InterfaceDescriptor {
        InterfaceDescriptor::new("HttpRequestOptions", None)
            .field(FieldDescriptor::of::<String>("url"))
            .field(FieldDescriptor::of::<HttpMethod>("method"))
            .field(FieldDescriptor::of::<Option<LuaBinaryInput>>("body"))
            .field(FieldDescriptor::of::<Option<HttpHeaderMap>>("headers"))
            .field(FieldDescriptor::of::<Option<HttpHeaderMap>>("cookies"))
    }
}
...
impl DescribeInterface for HttpRateLimitOptions {
    fn interface_descriptor() -> InterfaceDescriptor {
        InterfaceDescriptor::new("HttpRateLimitOptions", None)
            .field(FieldDescriptor::of::<String>("domain"))
            .field(
                FieldDescriptor::of::<Option<f64>>("requests_per_second")
                    .description("Defaults to `1.0`."),
            )
            .field(FieldDescriptor::of::<Option<Vec<u16>>>("retry_on"))
            .field(
                FieldDescriptor::of::<Option<u32>>("max_retries")
                    .description("Defaults to `3`."),
            )
            .field(
                FieldDescriptor::of::<Option<u64>>("backoff_ms")
...
impl DescribeInterface for HttpConcurrencyOptions {
    fn interface_descriptor() -> InterfaceDescriptor {
        InterfaceDescriptor::new("HttpConcurrencyOptions", None)
            .field(
                FieldDescriptor::of::<String>("host").description(
                    "Host or full URL; only the host portion is keyed, matching the\nrequest-time lookup in [`extract_domain`].",
                ),
            )
            .field(
                FieldDescriptor::of::<u32>("max_in_flight")
                    .description("Maximum simultaneous in-flight requests to this host. Must be at least 1."),
            )
    }
}
...
impl DescribeInterface for HttpResponse {
    fn interface_descriptor() -> InterfaceDescriptor {
        InterfaceDescriptor::new("HttpResponse", None)
            .field(FieldDescriptor::of::<bool>("success"))
            .field(FieldDescriptor::of::<Option<u16>>("status_code"))
            .field(FieldDescriptor::of::<String>("status_message"))
            .field(FieldDescriptor::of::<HttpHeaderMap>("headers"))
            .field(FieldDescriptor::of::<HttpHeaderMap>("cookies"))
            .field(FieldDescriptor::of::<BodyBytes>("body"))
            .field(
                FieldDescriptor::of::<Option<String>>("error_kind")
                    .description("Transport error category when `success` is false."),
            )
            .field(FieldDescriptor::of::<u32>("retries"))
            .field(FieldDescriptor::of::<bool>("rate_limited"))
    }
}
...
fn encode_uri_component(_lua: &Lua, input: String) -> mlua::Result<String> {
    Ok(percent_encoding::utf8_percent_encode(&input, URI_COMPONENT_SET).to_string())
}
fn module_export() -> ModuleExport {
    ModuleBuilder::new("harmony/http", "Http", "http", |lua| {
        let table = lua.create_table()?;
    ...
    .scope_id(
        "harmony.http",
        "Make outbound HTTP requests to any server on the internet.",
        Danger::High,
    )
    .type_alias::<HttpHeaderMap>()
    .class::<HttpMethod>()
    .interface::<HttpRequestOptions>()
    .interface::<HttpRateLimitOptions>()
    .interface::<HttpConcurrencyOptions>()
    .interface::<HttpResponse>()
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("request")
            .param_type::<HttpRequestOptions>("options")
            .returns::<HttpResponse>()
            .yields(),
        |lua| harmony_core::resolve_caller(lua),
        |_, plugin_id, options| async move { request(plugin_id, options).await },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("set_rate_limit")
            .param_type::<HttpRateLimitOptions>("options")
            .returns::<()>()
            .yields(),
        |lua| harmony_core::resolve_caller(lua),
        |lua, plugin_id, options| async move { set_rate_limit(lua, plugin_id, options).await },
    )
    .async_function_with_prelude(
        ModuleFunctionDescriptor::new("set_max_in_flight")
            .description(
                "Register an in-flight cap for `host`. Tighten-only; reconfiguring mid-burst briefly allows old and new permits while the old semaphore drains.",
            )
            .param_type::<HttpConcurrencyOptions>("options")
            .returns::<()>()
            .yields(),
        |lua| harmony_core::resolve_caller(lua),
        |lua, plugin_id, options| async move { set_max_in_flight(lua, plugin_id, options).await },
    )
    .sync_function(
        ModuleFunctionDescriptor::new("encode_uri_component")
