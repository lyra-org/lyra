...
pub struct LocalScheduler {
    ...
    pending_thread_starts: RefCell<HashMap<(u64, usize), PendingThreadStart>>,
...
impl LocalScheduler {
    ...
    pub fn spawn_luau_thread(
        ...
        self.register_pending_thread_start(key, handle, args, None);
        handle
    }
    ...
    pub fn spawn_luau_thread_after(
        ...
        self.task_threads
            .borrow_mut()
            .insert(handle.id(), (vm, thread));
        self.register_pending_thread_start(key, handle, args, Some(wake_at));
        handle
    }
    ...
    pub fn cancel(&self, id: TaskId) -> bool {
        ...
        if cancelled {
            self.clear_pending_thread_start_for_task(id);
    ...
    pub fn cancel_luau_thread(&self, thread: &luau::Thread) -> bool {
        let key = luau_thread_key(thread);
        self.pending_thread_starts.borrow_mut().remove(&key);
    ...
    fn prepare_luau_thread_start_args(
        ...
        let should_consume = self
            ...
            .borrow()
            .get(&key)
            .is_some_and(|pending_start| start_wake_is_not_later(wake_at, pending_start.wake_at));
        if !should_consume {
            return args;
        }

        let Some(pending_start) = self.pending_thread_starts.borrow_mut().remove(&key) else {
            return args;
        };
        let _ = self.cancel(pending_start.handle.id());

        let mut merged_args = pending_start.args;
        merged_args.append(&mut args);
        merged_args
    }

    fn register_pending_thread_start(
        &self,
        key: (u64, usize),
        handle: TaskHandle,
        args: Vec<luau::Value>,
        ...
        if self
            ...
            .borrow()
            .get(&handle.id())
            .is_some_and(|(_, thread)| !thread.is_started())
            && !self.pending_thread_starts.borrow().contains_key(&key)
        {
            self.pending_thread_starts.borrow_mut().insert(
                key,
                PendingThreadStart {
                    handle,
                    args,
    ...
    fn clear_pending_thread_start_for_task(&self, id: TaskId) {
        self.pending_thread_starts
            .borrow_mut()
            .retain(|_, pending_start| pending_start.handle.id() != id);
    }

    fn cancel_externally_started_thread_starts(&self) {
        let started = self
            .pending_thread_starts
            .borrow()
            .iter()
            .filter_map(|(key, pending_start)| {
                self.task_threads
                    .borrow()
                    .get(&pending_start.handle.id())
                    .is_some_and(|(_, thread)| thread.is_started())
                    .then_some((*key, pending_start.handle))
            })
            .collect::<Vec<_>>();

        for (key, handle) in started {
            self.pending_thread_starts.borrow_mut().remove(&key);
            let _ = self.cancel(handle.id());
        }
    }

    fn clear_stale_pending_thread_starts(&self) {
        self.pending_thread_starts
            .borrow_mut()
            .retain(|_, pending_start| {
                let Some(snapshot) = self.scheduler.borrow().snapshot(pending_start.handle.id())
                else {
                    return false;
                };
