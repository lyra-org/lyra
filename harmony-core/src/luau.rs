...
mod tests {
    ...
    fn completed_thread_returns_values_and_removes_task() -> Result<()> {
        ...
        let values = drive_thread(&tokio_runtime, &scheduler, &thread, options(Duration::from_secs(1)))?;

        assert_eq!(
            values,
            vec![
                luau::Value::Integer(42),
    ...
    fn failed_thread_returns_error_and_removes_task() -> Result<()> {
        ...
        let error = drive_thread(&tokio_runtime, &scheduler, &thread, options(Duration::from_secs(1)))
    ...
    fn delayed_thread_completes_after_wake() -> Result<()> {
        ...
        assert_eq!(values, vec![luau::Value::Integer(7)]);
        Ok(())
    }
}
