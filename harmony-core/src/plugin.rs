...
/// Normalize entries and enforce per-plugin invariants. Cross-plugin
/// resolution (required deps satisfied, no cycles) lives in
/// `resolve_dependencies` after all manifests are loaded.
...
impl PluginManager {
    ...
    fn resolve_dependencies(&mut self) -> Vec<PluginLoadError> {
        ...
        // on A. Sort for deterministic error order.
        loop {
    ...
    /// Kahn's algorithm. Returns `(scheduled, unscheduled)`; ties in
    /// `scheduled` break alphabetically. `unscheduled` is non-empty iff
    /// the graph has a cycle, and lists ids in or downstream of one.
