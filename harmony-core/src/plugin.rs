...
pub struct PluginManifest {
    ...
    /// Plugin-to-plugin dependencies. Each entry is one logical
    /// requirement; alternatives within an entry are OR-satisfied.
    /// A bare string is shorthand for a single-alternative hard dep.
    #[serde(default)]
    pub dependencies: Vec<DependencyEntry>,
}

/// One manifest dependency entry. A bare string is the common case
/// (`"musicbrainz"` = required, no alternatives); the object form
/// expresses alternatives and/or the optional flag.
#[derive(Debug, Clone, Deserialize)]
#[serde(untagged)]
...
pub struct DependencyGroup {
    /// Plugin ids that satisfy this requirement (OR-semantics across
    /// the list). Empty arrays are rejected at load time.
    pub any_of: Vec<String>,
    /// `true` (default): missing/unresolved => the dependent plugin
    /// fails to load. `false`: missing alternatives are tolerated;
    /// installed alternatives still affect load order.
...
/// Normalized, validated form of a `DependencyEntry`. Built once at
/// load time so the topo-sort and resolution passes can iterate
/// uniform shapes without re-matching the untagged enum.
#[derive(Debug, Clone)]
...
pub struct LoadedPlugin {
    ...
    /// Normalized dependency entries. Built from `manifest.dependencies`
    /// at load time after per-plugin validation.
...
/// Normalize raw `DependencyEntry`s into the uniform `NormalizedDependency`
/// shape and validate per-plugin invariants: non-empty alternatives,
/// no self-reference, no duplicate alternatives within a group, ids match
/// the plugin-id character set, and caps on count/alternatives.
...
impl PluginManager {
    ...
    /// After per-plugin loads succeed, validate cross-plugin invariants:
    /// every required dependency group has at least one installed
    /// alternative, and the resulting graph is acyclic. Plugins that
    /// fail either check are removed from `self.plugins` so the runtime
    /// never sees a half-resolved set.
    fn resolve_dependencies(&mut self) -> Vec<PluginLoadError> {
        let mut errors = Vec::new();

        // Iteratively prune: dropping plugin A can leave plugin B's
        // required dep on A unsatisfied, so loop until stable. Sort
        // candidates so the order of reported errors is deterministic.
        loop {
        ...
        if let Some(cycle) = self.find_dependency_cycle() {
            ...
            errors.push(PluginLoadError::DependencyCycle { plugins: cycle });
        }

        errors
    }

    /// Kahn's algorithm over the resolved dep graph. Returns the set of
    /// plugin ids that could not be ordered (members of, or downstream
    /// of, at least one cycle). `None` means the graph is acyclic.
    fn find_dependency_cycle(&self) -> Option<Vec<String>> {
        let (in_degree, adjacency) = self.build_dependency_graph();
        ...
        while let Some(id) = ready.iter().next().cloned() {
            ...
            if let Some(successors) = adjacency.get(&id) {
                for successor in successors {
                    if let Some(deg) = remaining.get_mut(successor) {
                        *deg -= 1;
                        if *deg == 0 {
    ...
    /// Build the dep graph: `(in_degree per plugin, adjacency: pred -> [succ])`.
    /// Edges are emitted for every installed alternative across both
    /// required and optional groups — if a dep is installed, the
    /// dependent runs after it regardless of whether the dep was
    ...
    pub fn reload_plugin(
        ...
        // Required-dep check against the currently-installed set. Done
        // before mutating so a failing reload leaves the previous
        // version intact.
        for dep in &plugin.dependencies {
            if !dep.required {
                continue;
        ...
        // The reload could have introduced a cycle (new dep pointing
        // back through the graph). Probe the updated state and roll
        // back if so.
        if let Some(cycle) = self.find_dependency_cycle() {
            ...
            return Err(PluginLoadError::DependencyCycle { plugins: cycle });
        }
    ...
    /// Plugins in dependency-respecting execution order. Ties are broken
    /// alphabetically by id so the order is stable across runs (the
    /// underlying `HashMap` is not). Discovery and reload both reject
    /// dep cycles before insert, so Kahn's algorithm is guaranteed to
    /// visit every plugin here.
    pub fn topological_order(&self) -> Vec<LoadedPlugin> {
        ...
        let mut order: Vec<LoadedPlugin> = Vec::with_capacity(self.plugins.len());
        while let Some(id) = ready.iter().next().cloned() {
            ready.remove(&id);
        ...
        debug_assert_eq!(
            order.len(),
            self.plugins.len(),
            "topological_order missed plugins — cycle invariant broken in PluginManager"
        );
...
mod tests {
    ...
    #[test]
    fn topological_order_runs_deps_before_dependents() {
        let mgr = manager_with(vec![
            loaded_plugin("consumer", vec![required(&["provider"])]),
    ...
    fn topological_order_orders_after_every_installed_alternative() {
        // consumer requires (a OR b); both installed → consumer must come after both.
        let mgr = manager_with(vec![
    ...
    fn resolve_detects_and_prunes_cycle() {
        ...
        let errors = mgr.resolve_dependencies();
        assert!(
            errors.iter().any(|e| matches!(e, PluginLoadError::DependencyCycle { .. })),
