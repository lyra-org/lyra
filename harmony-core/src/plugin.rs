...
impl PluginManager {
    ...
    /// Kahn's algorithm over the resolved dep graph. Returns
    /// `(scheduled, unscheduled)` where `scheduled` lists plugin ids in
    /// a dependency-respecting order (ties broken alphabetically) and
    /// `unscheduled` lists ids that couldn't be visited because they
    /// participate in, or sit downstream of, a cycle. An acyclic graph
    /// yields an empty `unscheduled`.
